...
#include <inttypes.h> /* PRIx32, PRIu32 */
...
void SECURITY_ApplyTamperEarlyStartup(void)
{
  ...
  /**
    * Enable RCC APB clock of RTC for tamper register access
  ...
  /**
    ...
    */
  STM32_ATOMIC_CLEAR_BIT_32(RCC->RTCCR, RCC_RTCCR_RTCDRST);
...
void SECURITY_ApplyRamCfgEarlyStartup(void)
{
  /**
    ...
    */
  /* Clock RAMCFG */
  STM32_ATOMIC_SET_BIT_32(RCC->AHB1ENR, LL_AHB1_GRP1_PERIPH_RAMCFG & LL_AHB1_GRP1_PERIPH_ALL_EN_MASK);
  (void)STM32_READ_REG(RCC->AHB1ENR); /* Dummy read to ensure clocking */
...
/**
  * @brief  Enable the cryptography clocks.
  * @param  None
  * @retval None
  */
void SECURITY_EnableCryptoClocks(void)
{
  /**
    * - Stop all used Crypton S_HCLK using RCC
    * - Drive low pulse on RNG_HRESETn, SAES_HRESETn, HASH_HRESETn, PKA_HRESETn, CCB_HRESETn using RCC
    * - Start all used crypton clocks RNG_HCLK, HASH_HCLK, PKA_HCLK, CCB_HCLK except SAES_HCLK (PKA can start PKA RAM clear process)
    */
  /* -1- Disable the clock of all the used Crypto IPs */
  ...
  /* -2- Reset all the used Crypto IPs */
  HAL_RCC_RNG_Reset();
  HAL_RCC_SAES_Reset();
  HAL_RCC_HASH_Reset();
  HAL_RCC_PKA_Reset();
  ...
  /* SAES clock enable is performed after RNG is stable */
  HAL_RCC_HASH_EnableClock();
  HAL_RCC_PKA_EnableClock();
  /* CCB clock enable is not mandatory */
}

/**
  * @brief  Disable the cryptography clocks.
  * @param  None
  * @retval None
  */
void SECURITY_DisableCryptoClocks(void)
{
  /**
    * - Stop all used Crypton S_HCLK using RCC
    * - Drive low pulse on RNG_HRESETn, SAES_HRESETn, HASH_HRESETn, PKA_HRESETn, CCB_HRESETn using RCC
...
void SECURITY_CheckStaticProtections(void)
{
  ...
  const uint32_t boot_addr_expected = (uint32_t)ROT_VTOR_ADDR;
  ob_value = (uint32_t) HAL_FLASH_ITF_OB_GetBootAddr(flash);
  if (ob_value != boot_addr_expected)
  {
    ROT_LOG_ERR("* boot address KO: expected [0x%" PRIx32 "], OB [0x%" PRIx32 "]", boot_addr_expected, ob_value);
    Error_Handler();
  }
  SECURITY_PROTECTION_LOG("- boot address OK [0x%" PRIx32 "]", boot_addr_expected);
...
#if defined(OEMIROT_OB_WRP_CHECK)
  /* Check OB write protection */
  for (i = WRP_CFG_START; i <= WRP_CFG_END; i++)
  {
    if (HAL_FLASH_ITF_OB_IsEnabledPageWRP(flash, HAL_FLASH_BANK_1, i) != HAL_FLASH_ITF_OB_WRP_PAGE_PROTECTED)
    {
      ROT_LOG_ERR("* wrp1 KO: wrp missing on page %" PRIu32, i);
      Error_Handler();
    }
  }
  SECURITY_PROTECTION_LOG("- wrp1 OK [start: 0x%" PRIx32 ", end: 0x%" PRIx32 "]",
                          (uint32_t)WRP_CFG_START,
                          (uint32_t)WRP_CFG_END);

#endif /* OEMIROT_OB_WRP_CHECK */

  /* Check OB hide protection */
  const uint32_t hdp_cfg_start = HDP_CFG_START;
  const uint32_t hdp_cfg_end = HDP_CFG_END;
  ...
  {
    ROT_LOG_ERR("* hdp1 KO: expected [start: 0x%" PRIx32 ", end: 0x%" PRIx32
                "], OB [start: 0x%" PRIx32 ", end: 0x%" PRIx32 "]",
                hdp_cfg_start,
                hdp_cfg_end,
                ob_start_page,
                ob_end_page);
  ...
  SECURITY_PROTECTION_LOG("- hdp1 OK [start: 0x%" PRIx32 ", end: 0x%" PRIx32 "]", hdp_cfg_start, hdp_cfg_end);
  ...
  const uint32_t lifecycle_expected = (uint32_t)OEMIROT_OB_RDP_LEVEL_MIN;
  /* Identify RDP_Level mini selected */
  for (i = 0U; i < NB_RDP_LEVEL; i++)
  {
    if (RDPLevelPrioList[i] == lifecycle_expected)
    {
      break;
    }
  }
  ...
  ob_value = (uint32_t)HAL_FLASH_ITF_OB_GetRDPLevel(flash);
  for (; i < NB_RDP_LEVEL; i++)
  {
    if (RDPLevelPrioList[i] == ob_value)
    {
      break;
    }
  }
  if (i >= NB_RDP_LEVEL)
  {
    ROT_LOG_ERR("* RDP_Level KO: minimum expected [0x%" PRIx32 "], OB [0x%" PRIx32 "]",
                lifecycle_expected,
                ob_value);
    Error_Handler();
  }
  SECURITY_PROTECTION_LOG("- RDP_Level OK [0x%" PRIx32 "]", (uint32_t)RDPLevelPrioList[i]);
}
