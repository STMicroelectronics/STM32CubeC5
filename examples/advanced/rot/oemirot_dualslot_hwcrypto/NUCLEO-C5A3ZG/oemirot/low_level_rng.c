/**
  ******************************************************************************
  * @file    low_level_rng.c
  * @brief   Low Level Interface module to use STM32 RNG Ip
  *          This file provides mbed-crypto random generataor
  *
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2023-2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
#include "boot_hal_cfg.h"
#include "low_level_rng.h"
#include "boot_hal_utils.h"

extern void Error_Handler(void);

/* Timeout for generation in millisecond */
#define RNG_TIMEOUT_VALUE        10U
#define RNG_GENERATION_TIMEOUT   10U

void RNG_Init(void)
{
  uint32_t dummy;
  uint32_t tickstart = 0U;
  hal_rng_handle_t *p_hrng = NULL;

  /**
    * - Configure RNG using CONDRST and NIST configuration
    * - Enable RNG and wait for RNG_DR.DRDY=1 in RNG_SR with SEIS=0 (No seed error)
    * - Start SAES_HCLK (RNG BUS is used to seed PRNG in SAES)
    * NB: this function stops here to allow using the HAL drivers to initialize
    *     SAES & PKA with correct application settings
    */

  /* -1- */

  p_hrng = mx_rot_rng_init();

  if (p_hrng == NULL)
  {
    Error_Handler();
  }

  /* Get the RNG instance for LL accesses */

  /* Disable Auto reset*/
  HAL_RNG_DisableAutoReset(p_hrng);

  /* Wait for conditioning reset process to be completed */
  tickstart = HAL_GetTick();
  while (LL_RNG_IsEnabledCondReset(RNG) == 1UL)
  {
    if ((HAL_GetTick() - tickstart) > RNG_TIMEOUT_VALUE)
    {
      Error_Handler();
    }
  }

  /* -2- */
  /* Enable the RNG Peripheral for the following handlings */
  LL_RNG_Enable(RNG);

  /* Get tick */
  tickstart = HAL_GetTick();

  /* Check if data register contains valid random data */
  while ((LL_RNG_IsActiveFlag_DRDY(RNG) == 0U))
  {
    if ((HAL_GetTick() - tickstart) > RNG_TIMEOUT_VALUE)
    {
      /* New check to avoid false timeout detection in case of preemption */
      if (LL_RNG_IsActiveFlag_DRDY(RNG) == 0U)
      {
        Error_Handler();
      }
      else if (LL_RNG_IsActiveFlag_SEIS(RNG) != 0U)
      {
        Error_Handler();
      }
    }
  }
  if (LL_RNG_IsActiveFlag_SEIS(RNG) != 0U)
  {
    Error_Handler();
  }

  /* -3- */
  /* Enable SAES clock after RNG is stable */
  HAL_RCC_SAES_EnableClock();

  /** Disable the RNG Peripheral
    * HAL_RNG_DeInit cannot be used as it sets the HAL state to RESET.
    */
  LL_RNG_Disable(RNG);

  /* first random number generated after setting the RNGEN bit must not be used */
  (void)HAL_RNG_GenerateRandomNumber(p_hrng, &dummy, 1U, RNG_GENERATION_TIMEOUT);
}

void RNG_GetBytes(uint8_t *output, size_t length, size_t *output_length)
{
  int32_t ret = 0;
  uint32_t tries = 0U;
  uint32_t i = 0U;
  __IO uint8_t random[4U] = {0U};
  uint32_t *p_rnd = (uint32_t *)random;
  *output_length = 0;
  hal_rng_handle_t *p_hrng = mx_rot_rng_gethandle();

  /* Get Random byte */
  while ((*output_length < length) && (ret == 0))
  {
    if (HAL_RNG_GenerateRandomNumber(p_hrng, p_rnd, 1U, RNG_GENERATION_TIMEOUT) != HAL_OK)
    {
      /* retry when random number generated are not immediately available */
      if (tries < 3U)
      {
        (void)HAL_RNG_RecoverSeedError(p_hrng);
        tries++;
      }
      else
      {
        ret = -1;
      }
    }
    else
    {
      for (i = 0U; (i < ARRAY_SIZE(random)) && (*output_length < length) ; i++)
      {
        *output++ = random[i];
        *output_length += 1;
        random[i] = 0;
      }
    }
  }

  /* Ensure no errors occurred during RNG operation */
  if ((LL_RNG_IsActiveFlag_CECS(RNG) != 0UL)
      || (LL_RNG_IsActiveFlag_SECS(RNG) != 0UL))
  {
    *output_length = 0;
  }
}
