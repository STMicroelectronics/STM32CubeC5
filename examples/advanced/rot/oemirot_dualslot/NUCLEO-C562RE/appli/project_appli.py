...
def get_ini_path() -> str:
    """
    Returns the normalized absolute path to the 'project_appli.ini' configuration file.

    The path is constructed by joining the directory of the current file with
    './project_appli.ini', and then normalizing it to remove any redundant
    separators or up-level references.

    Returns:
        str: The normalized path to the 'project_appli.ini' file.
    """
    return os.path.normpath(os.path.join(os.path.dirname(__file__), './project_appli.ini'))
...
from helper.project.generator import create_project, ProjectType
...
def main():
    rot_logger = ROT_Logger('application.log')
    project_app = create_project(ProjectType.OEMIROT_APPLICATION, project_ini_path, rot_logger)
    project_app.process_args()
