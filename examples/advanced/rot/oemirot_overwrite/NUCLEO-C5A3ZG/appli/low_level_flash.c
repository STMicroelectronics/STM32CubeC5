...
#define FLASH_TIMEOUT_VALUE         (100U)
...
/**
  ...
  */
struct arm_flash_dev_s
{
  low_level_device_t *dev;
  ARM_FLASH_INFO *data;       /*!< FLASH memory device data */
};

typedef struct arm_flash_dev_s arm_flash_dev_t;
/* Private variables ------------------------------------------------------------------*/
...
/**
  ...
  */
hal_flash_handle_t *p_flash_handle = NULL;
/* Private functions ------------------------------------------------------------------*/
/**
...
static bool is_range_valid(arm_flash_dev_t *flash_dev,
                           uint32_t offset)
{
...
/**
  ...
  * \return       Returns true if param is a page erasable, false
  *               otherwise.
  */
static bool is_erase_allow(arm_flash_dev_t *flash_dev,
                           uint32_t param)
{
...
/**
  ...
  *               otherwise.
  */
static bool is_write_allow(arm_flash_dev_t *flash_dev,
...
static bool is_write_aligned(arm_flash_dev_t *flash_dev,
                             uint32_t param)
{
...
/**
  ...
  *               otherwise.
  */
static bool is_erase_aligned(arm_flash_dev_t *flash_dev,
                             uint32_t param)
{
  /*  2 pages */
...
static ARM_FLASH_INFO ARM_FLASH0_DEV_DATA =
{
  ...
  .page_size      = 0U,                                     /* No page size defined */
...
static int32_t Flash_Initialize(ARM_Flash_SignalEvent_t cb_event)
{
  ...
  p_flash_handle = mx_flash_init();
  if (p_flash_handle == NULL)
  {
...
{
  uint32_t flash_addr = FLASH_BASE + off;

  /* Check Flash memory boundaries */
  if (is_range_valid(&ARM_FLASH0_DEV, off + size_byte - 1U) != true)
  {
#if defined(__ARMCC_VERSION)
    printf(" Data read not allowed %x \r\n", (flash_addr + size_byte - 1U));
#else
    printf(" Data read not allowed %lx \r\n", (flash_addr + size_byte - 1U));
  ...
  /*  do a memcpy */
#if defined(__ARMCC_VERSION)
  printf("read %x n=%x \r\n", flash_addr, size_byte);
#else
  printf("read %lx n=%lx \r\n", flash_addr, size_byte);
#endif /* __ARMCC_VERSION */

  memcpy(data, (void *)flash_addr, size_byte);
...
static int32_t Flash_ProgramData(uint32_t off, const void *data, uint32_t size_byte)
{
  ...
  */
  if ((!is_range_valid(&ARM_FLASH0_DEV, off + size_byte - 1U))
      || (!is_write_aligned(&ARM_FLASH0_DEV, off))
      || (!is_write_aligned(&ARM_FLASH0_DEV, size_byte))
      || (!is_write_allow(&ARM_FLASH0_DEV, off, size_byte))
      || (size_byte == 0U)
     )
  {
#if defined(__ARMCC_VERSION)
    printf("failed flash program (parameters) %x\r\n", flash_addr);
#else
    printf("failed flash program (parameters) %lx\r\n", flash_addr);
#endif /* __ARMCC_VERSION */

    return ARM_DRIVER_ERROR_PARAMETER;
  }

  if (HAL_FLASH_ITF_Unlock(p_flash_handle->instance) != HAL_OK)
  {
    return ARM_DRIVER_ERROR_SPECIFIC;
  }

  if (HAL_FLASH_SetProgrammingMode(p_flash_handle, programming_mode) != HAL_OK)
  {
    return ARM_DRIVER_ERROR_SPECIFIC;
  }
  ...
  do
  {
    memcpy(buf, (void *)((uint32_t)data + copied_size), FLASH_PROGRAM_UNIT);
    /* Avoid to write a quadword of 0xFF in flash*/
    if ((buf[0U] != UINT32_MAX) || (buf[1U] != UINT32_MAX) || (buf[2U] != UINT32_MAX) || (buf[3U] != UINT32_MAX))
    {
      if (HAL_FLASH_ProgramByAddr(p_flash_handle,
                                  flash_addr,
                                  (uint32_t const *)&buf[0U],
                                  sizeof(buf),
                                  FLASH_TIMEOUT_VALUE) != HAL_OK)
      {
#if defined(__ARMCC_VERSION)
        printf("failed flash program %x\r\n", flash_addr);
#else
        printf("failed flash program %lx\r\n", flash_addr);
#endif /* __ARMCC_VERSION */
        return ARM_DRIVER_ERROR_SPECIFIC;
      }
    }
  ...
  if (HAL_FLASH_ITF_Lock(p_flash_handle->instance) != HAL_OK)
  {
    return ARM_DRIVER_ERROR_SPECIFIC;
  }

  /* Check Programming operation has been done in the expected mode */
  if (HAL_FLASH_GetProgrammingMode(p_flash_handle) != programming_mode)
  {
    return ARM_DRIVER_ERROR;
  }
...
static int32_t Flash_ErasePage(uint32_t off)
{
  ...
  if (HAL_FLASH_EraseByAddr(p_flash_handle, flash_addr, FLASH_PAGE_SIZE, FLASH_TIMEOUT_VALUE) != HAL_OK)
  {
    printf("erase failed \r\n");
    return ARM_DRIVER_ERROR;
  }
  ...
  for (i = 0; i < (FLASH_PAGE_SIZE / 4); i++)
  {
    if (pt[i] != 0xffffffff)
    {
      BOOT_LOG_FLASH("erase failed off %x %x %x\r\n", off, &pt[i], pt[i]);
      return ARM_DRIVER_ERROR;
    }
  }
#endif /* CHECK_ERASE */
