...
app_status_t app_process(void)
{
  ...
  /** ########## Step 2 ##########
    * Transmits the start message in DMA mode using MX_UARTx.
    */
  UART_Transmit_DMA(TxStartMessage, (sizeof(TxStartMessage) - 1U));
  ...
  /** ########## Step 4 ##########
    * Starts the reception in DMA mode using MX_UARTx.
    */
  ...
  /** The detection of new line ('\n') character or the amount of data elements is received marks the end
    * of the reception.
    */
  UART_ReceiveUntilCM_DMA(DataBuffer, BUFFER_SIZE, '\n');
  ...
  UART_Transmit_DMA(DataBuffer, BUFFER_SIZE);
