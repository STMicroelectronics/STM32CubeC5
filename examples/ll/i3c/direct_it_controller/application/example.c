...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
volatile uint8_t DAAProcessComplete; /* Set to 1 when the Dynamic Address Assignment (DAA) process reaches completion */
volatile uint8_t TransferComplete;   /* Set to 1 when a data transfer operation is complete                           */
volatile uint8_t DAAFailedAttent;    /* Set to 1 when a DAA attempt is failed                                         */
...
app_status_t app_process(void)
{
  ...
  if (DAAProcessComplete == 1U)
  {
    ...
    /** ########## Step 3 ##########
      * Build the control buffer for the upcoming sequence of direct MX_I3Cx CCC (Common Command Code) operations.
    ...
    /** ########## Step 4 ##########
      * Performs the direct CCC transfer in interrupt mode.
      * This step sends the SETMWL and SETMRL payload bytes stored in DirectWriteCCC and stores the
      * GETMWL, GETMRL, GETPID, GETBCR, GETDCR, and GETSTATUS responses in RxBuffer.
      */

    I3C_Transfer_IT(ControlBuffer,
                    CONTROL_BUFFER_SIZE,
...
{
  /* Asynchronous processing for transfer completion */
  TransferComplete = 1U;
}
