...
# __Example: *ll_i3c_ibi_target*__
...
How to send In-Band Interrupts (IBI) as an I3C target using the LL Layer in interrupt (IT) mode.
...
This example demonstrates how the target application performs IBI signaling and interrupt-driven using the LL Layer API in interrupt (IT) mode.
...
__Step 2__: Perform Dynamic Address Assignment (DAA): activate notification, wait for DAA completion (interrupt-driven) and validate assigned dynamic address.
...
- IBI target configuration: payload size set to 3 byte and pending read MDB (Mandatory Data Byte) feature disabled.

__Multi-target setup and preparation__:

To simulate multiple I3C targets for IBI, prepare each board as follows:

- Before connecting to the I3C bus, set a unique MIPI (Mobile Industry Processor Interface) identifier for each target.

To test this example with the controller, you can use the corresponding *ll_i3c_ibi_controller* example pack.
...
<!--
@startuml
@startditaa{doc/example_ll_i3c_ibi_target-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_ll_i3c_ibi_target-setup](doc/example_ll_i3c_ibi_target-setup.png)
