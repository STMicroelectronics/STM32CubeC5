...
# __Example: *ll_i3c_direct_it_target*__

**Example version:** 2.0.0

[![User Manual](doc/read_the-UM.svg)](https://dev.st.com/stm32cube-docs/examples/arch-v1/en/index.html "An offline version is also available in the STM32Cube firmware package.")

How to handle direct I3C notifications and validate Max Read Length (MRL) / Max Write Length (MWL) updates in interrupt (IT) mode with the LL (Low-Layer) API.


## __1. Detailed scenario__

In this example, the target application receives its dynamic address (DAA) from the controller, then checks and validates the MRL/MWL update flags.
...
__Step 2__: Activates target notifications, waits for Dynamic Address Assignment (DAA) completion, and verifies the assigned dynamic address when error handling is enabled.

__Step 3__: Validates the Max Read Length (MRL) and Max Write Length (MWL) values reported through the update callbacks.

The communication status is reported via the status LED and the variable `ExecStatus`.

__End of example__: The scenario is executed once. If a mismatch is detected on MRL or MWL, or on the dynamic address when `USE_LL_APP_ERROR` is enabled, the application reports an error status and enters the error handler (status LED blinking).
...
To test this example with the controller, you can use the corresponding *ll_i3c_direct_it_controller* example pack.
...
<!--
@startuml
@startditaa{doc/example_ll_i3c_direct_it_target-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_ll_i3c_direct_it_target-setup](doc/example_ll_i3c_direct_it_target-setup.png)
