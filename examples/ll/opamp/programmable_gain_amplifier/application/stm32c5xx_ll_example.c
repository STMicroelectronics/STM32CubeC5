...
#define ADC_DISABLE_TIMEOUT_MS            (2UL)       /* MX_ADCx disable time-out value (unit: milli seconds)         */
...
inline system_status_t ADC_Activate(void)
{
  ...
  while (LL_ADC_IsActiveFlag_ADRDY(MX_ADCx) == 0UL)
  {
    ...
    if (LL_ADC_IsEnabled(MX_ADCx) == 0UL)
    {
      LL_ADC_Enable(MX_ADCx);
    }
...
{
  /* Enables the MX_ADCx group regular end of sequence conversion interrupt and overrun interrupt when
     USE_LL_APP_ERROR == 1 */
#if defined(USE_LL_APP_ERROR) && (USE_LL_APP_ERROR == 1)
  LL_ADC_EnableIT(MX_ADCx, (LL_ADC_IT_EOC | LL_ADC_IT_OVR));
#else
  LL_ADC_EnableIT(MX_ADCx, LL_ADC_IT_EOC);
