...
*** Settings ***
Documentation       Automatic test for example_ll_opamp_standalone.
...
Example Step 2
    [Documentation]    The OPAMP auto calibration is performed to ensure accurate measurements
...
Example Step 3
    ...
    Comment
    ...    The purpose of the test is to demonstrate a use case in standalone mode, for which we have selected comparator mode.
    ...
    Check Voltage    MX_EXAMPLE_OPAMP_VOUT_PIN    0V    0.100V    timeout=3s

    Comment    If VINP is higher than VINM, the OPAMP output VOUT should be at 3.3 V.
    Write Voltage    MX_EXAMPLE_OPAMP_VINP_PIN    3300mV
    Write Pin    MX_EXAMPLE_OPAMP_VINM_PIN    0
    Check Voltage    MX_EXAMPLE_OPAMP_VOUT_PIN    3.1V    3.3V    timeout=3s
