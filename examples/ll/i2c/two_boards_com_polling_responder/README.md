...
# __Example: *ll_i2c_two_boards_com_polling_responder*__
...
The example implements the responder's code.
...
__Step 2__: The responder expects to receive a message as a null-terminated string from the controller board, in blocking mode, within a specific timeout period.
...
> **_NOTE:_** when an error occurs in the reception or transmission process, the responder restarts the reception process. The error_handler() function is called when the maximum number of attempts is reached and an error status is reported.
...
- The selected GPIO pins support the I2C alternate function. They are configured in open drain mode with no pull-up neither pull-down.
...
<!--
@startuml
@startditaa{doc/example_ll_i2c_two_boards_com_polling_responder-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_ll_i2c_two_boards_com_polling_responder-setup](doc/example_ll_i2c_two_boards_com_polling_responder-setup.png)
