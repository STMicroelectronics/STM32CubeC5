...
/** brief:  MX_I2Cx receive an amount of data in blocking mode (polling mode).
  ...
  */
inline system_status_t I2C_Receive(void *p_rx_buffer,
                                   uint32_t size_byte,
                                   uint32_t timeout_ms)
{
  ...
  LL_I2C_AcknowledgeEnable(MX_I2Cx);

  /* Wait until ADDR flag is set */
  return_status = I2C_WaitOnFlagUntilTimeout(LL_I2C_ISR_ADDR, I2C_FLAG_INACTIVE, timeout_ms);
  if (return_status == SYSTEM_OK)
  {
    LL_I2C_ClearFlag_ADDR(MX_I2Cx);

    /* Wait until DIR flag is reset receiver mode */
    return_status = I2C_WaitOnFlagUntilTimeout(LL_I2C_ISR_DIR, I2C_FLAG_ACTIVE, timeout_ms);
    if (return_status == SYSTEM_OK)
    {
      while (rx_count > 0U)
      {
      ...
      if (return_status == SYSTEM_OK)
      {
        ...
        if (return_status == SYSTEM_OK)
        {
          ...
          return_status = I2C_WaitOnFlagUntilTimeout(LL_I2C_ISR_BUSY, I2C_FLAG_ACTIVE, timeout_ms);
        }
      }
    }
  }
...
/** brief:  MX_I2Cx transmit an amount of data in blocking mode (polling mode).
  ...
  * retval: system status (see system_status_t).
  */
inline system_status_t I2C_Transmit(const void *p_tx_buffer,
                                    uint32_t size_byte,
                                    uint32_t timeout_ms)
{
  ...
  if (return_status == SYSTEM_OK)
  {
    ...
    if (LL_I2C_IsEnabledClockStretching(MX_I2Cx) == 0UL)
    {
    ...
    if (return_status == SYSTEM_OK)
    {
      ...
      return_status = I2C_WaitOnFlagUntilTimeout(LL_I2C_ISR_DIR, I2C_FLAG_INACTIVE, timeout_ms);
      if (return_status == SYSTEM_OK)
      {
        while (tx_count > 0U)
        {
        ...
        if (return_status == SYSTEM_OK)
        {
          return_status = I2C_WaitOnFlagUntilTimeout(LL_I2C_ISR_NACKF, I2C_FLAG_INACTIVE, timeout_ms);
          if (return_status == SYSTEM_OK)
          {
