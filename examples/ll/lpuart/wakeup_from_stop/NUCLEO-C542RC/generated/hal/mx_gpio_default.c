...
system_status_t mx_gpio_default_init(void)
{
  ...
  /*
    GPIO pin labels :
    PB0   ---------> PB0, MX_STATUS_WAKEUP
    */

  /* Initialize the output state */
  /* LL_GPIO_WriteOutputPin(PB0_PORT, PB0_PIN, LL_GPIO_PIN_RESET); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output speed (Low, Medium, High or Very-High) */
  /* LL_GPIO_SetPinSpeed(PB0_PORT, PB0_PIN, LL_GPIO_SPEED_FREQ_LOW); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output type (Push-Pull or Open-Drain) */
  /* LL_GPIO_SetPinOutputType(PB0_PORT, PB0_PIN, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */

  /* Activate the Pull-up, Pull-down resistor, or No pull for the current IO */
  /* LL_GPIO_SetPinPull(PB0_PORT, PB0_PIN, LL_GPIO_PULL_NO); */ /* Configuration matches register reset state at startup. */

  /* Configure PB0 GPIO pin in output mode */
  LL_GPIO_SetPinMode(PB0_PORT, PB0_PIN, LL_GPIO_MODE_OUTPUT);
...
system_status_t mx_gpio_default_deinit(void)
{
  ...
  /* Reset the current IO output state */
  LL_GPIO_ResetOutputPin(PB0_PORT, PB0_PIN);
