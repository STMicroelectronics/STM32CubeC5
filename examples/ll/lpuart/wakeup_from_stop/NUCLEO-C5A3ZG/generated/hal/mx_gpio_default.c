...
system_status_t mx_gpio_default_init(void)
{
  ...
  /* PB0   --------->  PB0 */
  /* PB0   --------->  MX_STATUS_WAKEUP */

  /* Initialize the output state */
  /* LL_GPIO_WriteOutputPin(GPIOB, LL_GPIO_PIN_0, LL_GPIO_PIN_RESET); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output speed (Low, Medium, High or Very-High) */
  /* LL_GPIO_SetPinSpeed(GPIOB, LL_GPIO_PIN_0, LL_GPIO_SPEED_FREQ_LOW); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output type (Push-Pull or Open-Drain) */
  /* LL_GPIO_SetPinOutputType(GPIOB, LL_GPIO_PIN_0, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */
...
system_status_t mx_gpio_default_deinit(void)
{
  ...
  /* Configure the default Alternate Function in current IO */
  LL_GPIO_SetAFPin_8_15(GPIOB, LL_GPIO_PIN_0, LL_GPIO_AF_0);
