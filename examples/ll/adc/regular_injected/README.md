...
# __Example: *ll_adc_regular_injected*__
...
This example is based on the ADC LL API.
...
__Step 4__: The ADC conversion is started in interrupt mode for injected channels (one conversion after each MX_ADCx start).
...
__End of example__: The ADC conversion is repeated every second endlessly (step 4 to 5 is running in a loop) as long as there is no error. The example status is reported via the variable **`ExecStatus`**, and the status LED remains turned on in case of success.
...
- Software trigger
- Start in intpolling mode for ADC channels regular group
- Start with interrupt mode for ADC channels injected group
