...
/**
  * brief:  Start the MX_ADCx regular channels conversion in polling mode.
  * retval: system status (see system_status_t)
  */
system_status_t ADC_REG_StartConversion(void);

/**
  *  brief:  Start the MX_ADCx injected channels conversion in interrupt mode.
  */
void ADC_INJ_StartConversion_IT(void);
