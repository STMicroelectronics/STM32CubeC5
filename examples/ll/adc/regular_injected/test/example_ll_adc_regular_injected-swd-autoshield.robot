...
*** Settings ***
Documentation       Automatic test for example_ll_adc_regular_injected.
...
Example Step 2
    [Documentation]    Start the ADC channels conversion
    Comment    No check possible
...
Example Step 3
    ...
    Check Integer Variable    GPIO_mV    0    100    timeout=3s
    Write Voltage    ANALOG_PIN    1.200V
    Check Integer Variable    GPIO_mV    1100    1300    timeout=3s
    Write Voltage    ANALOG_PIN    3.200V
    Check Integer Variable    GPIO_mV    3100    3300    timeout=3s
    # ADC group injected conversions
    Check Integer Variable    VrefInt_mV    ${voltage_below_vref}    ${voltage_over_vref}    timeout=2s
    Check Integer Variable    Temperature_DegC    ${Temp_DegreeCelsius_below}    ${Temp_DegreeCelsius_over}    timeout=2s
    # This sleep is added to ensure that 20S (the duration of the process execution) is completed
    Sleep    10s
End Of Example
