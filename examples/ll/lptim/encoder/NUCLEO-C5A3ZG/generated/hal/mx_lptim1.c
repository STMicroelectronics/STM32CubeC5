...
LPTIM_TypeDef *mx_lptim1_init(void)
{
  ...
  LL_AHB2_GRP1_EnableClock(LL_AHB2_GRP1_PERIPH_GPIOA | LL_AHB2_GRP1_PERIPH_GPIOD);
  ...
  /**
    [GPIO Pin] ------> [Signal Name] ------> [Labels]
       ...
       PD11    ------>   LPTIM1_IN2   ------>  PD11
    **/

  /* Configure IO output speed (Low, Medium, High or Very-High) */
  LL_GPIO_SetPinSpeed(PD11_PORT, PD11_PIN, LL_GPIO_SPEED_FREQ_VERY_HIGH);

  /* Configure IO output type (Push-Pull or Open-Drain) */
  /* LL_GPIO_SetPinOutputType(PD11_PORT, PD11_PIN, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */

  /* Activate the Pull-up, Pull-down resistor, or No pull for the current IO */
  /* LL_GPIO_SetPinPull(PD11_PORT, PD11_PIN, LL_GPIO_PULL_NO); */ /* Configuration matches register reset state at startup. */

  /* Configure the Alternate Function in current IO */
  LL_GPIO_SetAFPin_8_15(PD11_PORT, PD11_PIN, LL_GPIO_AF_1);

  /* Configure IO direction mode (Input, Output, Alternate or Analog) */
  LL_GPIO_SetPinMode(PD11_PORT, PD11_PIN, LL_GPIO_MODE_ALTERNATE);
...
void mx_lptim1_deinit(void)
{
  ...
  /* Configure IO in Analog Mode */
  LL_GPIO_SetPinMode(PD11_PORT, PD11_PIN, LL_GPIO_MODE_ANALOG);

  /* Configure the default Alternate Function in current IO */
  LL_GPIO_SetAFPin_8_15(PD11_PORT, PD11_PIN, LL_GPIO_AF_0);

  /* Configure the default value for IO Speed */
  LL_GPIO_SetPinSpeed(PD11_PORT, PD11_PIN, LL_GPIO_SPEED_FREQ_LOW);

  /* Configure the default value IO Output Type */
  LL_GPIO_SetPinOutputType(PD11_PORT, PD11_PIN, LL_GPIO_OUTPUT_PUSHPULL);

  /* Deactivate the Pull-up and Pull-down resistor for the current IO */
  LL_GPIO_SetPinPull(PD11_PORT, PD11_PIN, LL_GPIO_PULL_NO);

  /* Reset the IO output state */
  LL_GPIO_WriteOutputPin(PD11_PORT, PD11_PIN, LL_GPIO_PIN_RESET);
}
