...
LPTIM_TypeDef *mx_lptim1_init(void)
{
  ...
  /* LL_GPIO_SetPinSpeed(PA1_PORT, PA1_PIN, LL_GPIO_SPEED_FREQ_LOW); */ /* Configuration matches register reset state at startup. */
  ...
  /* Configure IO output speed (Low, Medium, High or Very-High) */
  /* LL_GPIO_SetPinSpeed(PB4_PORT, PB4_PIN, LL_GPIO_SPEED_FREQ_LOW); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output type (Push-Pull or Open-Drain) */
  /* LL_GPIO_SetPinOutputType(PB4_PORT, PB4_PIN, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */

  /* Activate the Pull-up, Pull-down resistor, or No pull for the current IO */
  /* LL_GPIO_SetPinPull(PB4_PORT, PB4_PIN, LL_GPIO_PULL_UP); */ /* Configuration matches register reset state at startup. */

  /* Configure the Alternate Function in current IO */
  LL_GPIO_SetAFPin_0_7(PB4_PORT, PB4_PIN, LL_GPIO_AF_4);

  /* Configure IO direction mode (Input, Output, Alternate or Analog) */
  /* LL_GPIO_SetPinMode(PB4_PORT, PB4_PIN, LL_GPIO_MODE_ALTERNATE); */ /* Configuration matches register reset state at startup. */
  ...
  LL_LPTIM_SetAutoReload(LPTIM1, 0x63);
  ...
  /* LL_LPTIM_OC_SetPolarity(LPTIM1, LL_LPTIM_CHANNEL_CH2, LL_LPTIM_OCPOLARITY_HIGH); */ /* Configuration matches register reset state at startup. */
  LL_LPTIM_Enable(LPTIM1);
  LL_LPTIM_WRITE_REG(LPTIM1, ICR, LL_LPTIM_ISR_CMP2OK);
  LL_LPTIM_OC_SetCompareCH2(LPTIM1, 74);
  while (LL_LPTIM_IsActiveFlag_CMP2OK(LPTIM1) == 0);
  LL_LPTIM_ClearFlag_CMP2OK(LPTIM1);
  LL_LPTIM_Disable(LPTIM1);
  /* LL_LPTIM_CC_SetChannelMode(LPTIM1, LL_LPTIM_CHANNEL_CH2, LL_LPTIM_CCMODE_OUTPUT_PWM); */ /* Configuration matches register reset state at startup. */
...
void mx_lptim1_deinit(void)
{
  ...
  /* ### GPIO deinitialization of LPTIM1: LPTIM1_CH2 ########################### */

  /* Configure IO in Analog Mode */
  LL_GPIO_SetPinMode(PB4_PORT, PB4_PIN, LL_GPIO_MODE_ANALOG);

  /* Configure the default Alternate Function in current IO */
  LL_GPIO_SetAFPin_0_7(PB4_PORT, PB4_PIN, LL_GPIO_AF_0);

  /* Configure the default value for IO Speed */
  LL_GPIO_SetPinSpeed(PB4_PORT, PB4_PIN, LL_GPIO_SPEED_FREQ_LOW);

  /* Configure the default value IO Output Type */
  LL_GPIO_SetPinOutputType(PB4_PORT, PB4_PIN, LL_GPIO_OUTPUT_PUSHPULL);

  /* Deactivate the Pull-up and Pull-down resistor for the current IO */
  LL_GPIO_SetPinPull(PB4_PORT, PB4_PIN, LL_GPIO_PULL_NO);

  /* Reset the IO output state */
  LL_GPIO_WriteOutputPin(PB4_PORT, PB4_PIN, LL_GPIO_PIN_RESET);
}
