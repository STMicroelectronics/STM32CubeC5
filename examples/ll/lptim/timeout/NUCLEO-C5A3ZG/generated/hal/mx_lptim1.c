...
LPTIM_TypeDef *mx_lptim1_init(void)
{
  ...
  /* LL_GPIO_SetPinSpeed(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_SPEED_FREQ_LOW); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output type (Push-Pull or Open-Drain) */
  /* LL_GPIO_SetPinOutputType(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */

  /* Activate the Pull-up, Pull-down resistor, or No pull for the current IO */
  /* LL_GPIO_SetPinPull(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_PULL_UP); */ /* Configuration matches register reset state at startup. */

  /* Configure the Alternate Function in current IO */
  LL_GPIO_SetAFPin_8_15(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_AF_3);

  /* Configure IO direction mode (Input, Output, Alternate or Analog) */
  /* LL_GPIO_SetPinMode(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_MODE_ALTERNATE); */ /* Configuration matches register reset state at startup. */
  ...
  LL_LPTIM_SetAutoReload(LPTIM1, 0x7D00);
  ...
  LL_LPTIM_OC_SetCompareCH1(LPTIM1, 32000);
  while (LL_LPTIM_IsActiveFlag_CMP1OK(LPTIM1) == 0);
  LL_LPTIM_ClearFlag_CMP1OK(LPTIM1);
  LL_LPTIM_Disable(LPTIM1);

  /*Configure External Trigger (ETR) input*/
  LL_LPTIM_ConfigTrigger(LPTIM1, LL_LPTIM_TRIG_SOURCE_GPIO, LL_LPTIM_TRIG_FILTER_NONE, LL_LPTIM_TRIG_POLARITY_RISING_FALLING);
...
void mx_lptim1_deinit(void)
{
  ...
  LL_GPIO_SetPinMode(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_MODE_ANALOG);

  /* Configure the default Alternate Function in current IO */
  LL_GPIO_SetAFPin_8_15(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_AF_0);

  /* Configure the default value for IO Speed */
  LL_GPIO_SetPinSpeed(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_SPEED_FREQ_LOW);

  /* Configure the default value IO Output Type */
  LL_GPIO_SetPinOutputType(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_OUTPUT_PUSHPULL);

  /* Deactivate the Pull-up and Pull-down resistor for the current IO */
  LL_GPIO_SetPinPull(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_PULL_NO);

  /* Reset the IO output state */
  LL_GPIO_WriteOutputPin(NETR53_2_PORT, NETR53_2_PIN, LL_GPIO_PIN_RESET);
