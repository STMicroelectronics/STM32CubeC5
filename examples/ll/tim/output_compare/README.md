...
This scenario demonstrates how to configure a timer to generate two signals PWM in output compare toggle mode with delays between them using the same frequency using LL API.
...
- The timer channels (say '1x' and '2x') are configured in Output Compare Toggle mode.
...
- To obtain a delay of 250 microseconds between two signals, the Output Compare (OC) toggle frequencies are configured as follows:
  TIM_CCR register value is equal to 1 for channel 1x
  TIM_CCR register value is equal to 251 for channel 2x
...
As Output compare preload is enabled, the pulse value is taken into account at the next update event (UEV), which occurs when the counter reaches the ARR or CRR value. To correctly synchronize both channels, the TIM_CCRx value is set to 1 (not 0) to trigger an update event on the first timer loop.
...
<details>
  ...
  <summary>Numerical calculations:</summary>
    ...
    The delay between two signals is calculated as follows:
      ...
      Delay = (TIM_CCRz - TIM_CCRy) / tim_cnt_ck (Where TIM_CCRz for channel 1x and TIM_CCRy for channel 2x)
      Delay = (251 - 1) / 1000000
      Delay = 250 us
