...
TIM_TypeDef *mx_tim2_init(void)
{
  ...
  /* LL_GPIO_SetPinSpeed(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_SPEED_FREQ_VERY_HIGH); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output type (Push-Pull or Open-Drain) */
  /* LL_GPIO_SetPinOutputType(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */

  /* Activate the Pull-up, Pull-down resistor, or No pull for the current IO */
  /* LL_GPIO_SetPinPull(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_PULL_NO); */ /* Configuration matches register reset state at startup. */

  /* Configure the Alternate Function in current IO */
  LL_GPIO_SetAFPin_0_7(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_AF_1);

  /* Configure IO direction mode (Input, Output, Alternate or Analog) */
  /* LL_GPIO_SetPinMode(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_MODE_ALTERNATE); */ /* Configuration matches register reset state at startup. */
...
void mx_tim2_deinit(void)
{
  ...
  LL_GPIO_SetPinMode(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_MODE_ANALOG);

  /* Configure the default Alternate Function in current IO */
  LL_GPIO_SetAFPin_0_7(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_AF_0);

  /* Configure the default value for IO Speed */
  LL_GPIO_SetPinSpeed(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_SPEED_FREQ_LOW);

  /* Configure the default value IO Output Type */
  LL_GPIO_SetPinOutputType(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_OUTPUT_PUSHPULL);

  /* Deactivate the Pull-up and Pull-down resistor for the current IO */
  LL_GPIO_SetPinPull(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_PULL_NO);

  /* Reset the IO output state */
  LL_GPIO_WriteOutputPin(DBGIN_SWO_PORT, DBGIN_SWO_PIN, LL_GPIO_PIN_RESET);
