...
TIM_TypeDef *mx_tim1_init(void)
{
  ...
  LL_TIM_EnableOnePulseMode(TIM1);
  ...
  LL_TIM_OC_SetCompareValue(TIM1, LL_TIM_OC_COMPARE_UNIT_1, 500);

  /* Channel 2 Configuration */
  /* LL_TIM_IC_SetSource(TIM1, LL_TIM_TIM1_TI2_GPIO); */ /* Configuration matches register reset state at startup. */
  LL_TIM_IC_Config(TIM1, LL_TIM_CHANNEL_CH2, LL_TIM_ACTIVEINPUT_DIRECT | LL_TIM_ICPSC_DIV1 | LL_TIM_IC_FILTER_FDIV1 | LL_TIM_IC_POLARITY_RISING);
  ...
  /* LL_GPIO_SetPinOutputType(GPIOA, PA9_PIN | PA8_PIN, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */
...
void mx_tim1_deinit(void)
{
  ...
  /* ### GPIO deinitialization of TIM1: TIM1_CH2,TIM1_CH1 ########################### */
  ...
  LL_GPIO_SetPinOutputType(GPIOA, PA9_PIN | PA8_PIN, LL_GPIO_OUTPUT_PUSHPULL);
  ...
  LL_GPIO_WriteOutputPin(PA9_PORT, PA9_PIN | PA8_PIN, LL_GPIO_PIN_RESET);
}
