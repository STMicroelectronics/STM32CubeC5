...
TIM_TypeDef *mx_tim1_init(void)
{
  ...
  LL_TIM_OC_SetCompareValue(TIM1, LL_TIM_OC_COMPARE_UNIT_1, 90);
  ...
  LL_TIM_OC_SetBreakMode(TIM1, LL_TIM_CHANNEL_CH2, LL_TIM_OCBREAKMODE_DELAY1);

  /* Compare Unit 2 Configuration */
  LL_TIM_OC_SetMode(TIM1, LL_TIM_CHANNEL_CH2, LL_TIM_OCMODE_PWM1);
  LL_TIM_OC_SetCompareValue(TIM1, LL_TIM_OC_COMPARE_UNIT_2, 90);
  /* Channel 3 Configuration */
  /* LL_TIM_OC_SetPolarity(TIM1, LL_TIM_CHANNEL_CH3, LL_TIM_OCPOLARITY_HIGH); */ /* Configuration matches register reset state at startup. */
  /* LL_TIM_OC_SetIdleState(TIM1, LL_TIM_CHANNEL_CH3, LL_TIM_OCIDLESTATE_RESET); */ /* Configuration matches register reset state at startup. */
  /* LL_TIM_OC_SetOverrideState(TIM1, LL_TIM_CHANNEL_CH3, LL_TIM_OCOVERRIDE_RESET); */ /* Configuration matches register reset state at startup. */
  LL_TIM_OC_SetBreakMode(TIM1, LL_TIM_CHANNEL_CH3, LL_TIM_OCBREAKMODE_DELAY2);

  /* Compare Unit 3 Configuration */
  LL_TIM_OC_SetMode(TIM1, LL_TIM_CHANNEL_CH3, LL_TIM_OCMODE_PWM1);
  LL_TIM_OC_SetCompareValue(TIM1, LL_TIM_OC_COMPARE_UNIT_3, 90);
  ...
  /* Break Configuration */
  /* LL_TIM_ConfigBRK(TIM1, LL_TIM_BREAK_POLARITY_LOW, LL_TIM_BREAK_FILTER_FDIV1, LL_TIM_BREAK_AFMODE_INPUT); */ /* Configuration matches register reset state at startup. */
  LL_TIM_SetBreakDelay(TIM1, LL_TIM_BREAK_DELAY1, 60);
  LL_TIM_SetBreakDelay(TIM1, LL_TIM_BREAK_DELAY2, 120);

  /* Break Input Sources Configuration */
  /* LL_TIM_SetBreakInputSourcePolarity(TIM1, LL_TIM_BREAK_INPUT_1, LL_TIM_TIM1_BRK_GPIO, LL_TIM_BREAK_INPUT_SRC_NONINVERTED); */ /* Configuration matches register reset state at startup. */
  /* LL_TIM_EnableBreakInputSource(TIM1, LL_TIM_BREAK_INPUT_1, LL_TIM_TIM1_BRK_GPIO); */ /* Configuration matches register reset state at startup. */

  /* Enable Break 1 Input */
  LL_TIM_EnableBreakInput(TIM1, LL_TIM_BREAK_INPUT_1);
  ...
  /* LL_GPIO_SetPinSpeed(PA10_PORT, PA10_PIN, LL_GPIO_SPEED_FREQ_LOW); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output type (Push-Pull or Open-Drain) */
  /* LL_GPIO_SetPinOutputType(GPIOA, PA8_PIN | PA9_PIN | PA10_PIN, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */
  ...
  /* LL_GPIO_SetPinPull(PA10_PORT, PA10_PIN, LL_GPIO_PULL_NO); */ /* Configuration matches register reset state at startup. */
  ...
  LL_GPIO_SetAFPin_8_15(PA10_PORT, PA10_PIN, LL_GPIO_AF_1);
  ...
  LL_GPIO_SetPinMode(PA10_PORT, PA10_PIN, LL_GPIO_MODE_ALTERNATE);
  ...
  /* Configure IO output speed (Low, Medium, High or Very-High) */
  /* LL_GPIO_SetPinSpeed(PA6_PORT, PA6_PIN, LL_GPIO_SPEED_FREQ_LOW); */ /* Configuration matches register reset state at startup. */

  /* Configure IO output type (Push-Pull or Open-Drain) */
  /* LL_GPIO_SetPinOutputType(PA6_PORT, PA6_PIN, LL_GPIO_OUTPUT_PUSHPULL); */ /* Configuration matches register reset state at startup. */

  /* Activate the Pull-up, Pull-down resistor, or No pull for the current IO */
  LL_GPIO_SetPinPull(PA6_PORT, PA6_PIN, LL_GPIO_PULL_UP);

  /* Configure the Alternate Function in current IO */
  LL_GPIO_SetAFPin_0_7(PA6_PORT, PA6_PIN, LL_GPIO_AF_1);
...
void mx_tim1_deinit(void)
{
  ...
  LL_GPIO_SetPinSpeed(PA10_PORT, PA10_PIN, LL_GPIO_SPEED_FREQ_LOW);

  /* Configure the default value IO Output Type */
  LL_GPIO_SetPinOutputType(GPIOA, PA8_PIN | PA9_PIN | PA10_PIN, LL_GPIO_OUTPUT_PUSHPULL);

  /* Deactivate the Pull-up and Pull-down resistor for the current IO */
  LL_GPIO_SetPinPull(PA8_PORT, PA8_PIN, LL_GPIO_PULL_NO);
  LL_GPIO_SetPinPull(PA9_PORT, PA9_PIN, LL_GPIO_PULL_NO);
  LL_GPIO_SetPinPull(PA10_PORT, PA10_PIN, LL_GPIO_PULL_NO);

  /* Reset the IO output state */
  LL_GPIO_WriteOutputPin(PA8_PORT, PA8_PIN | PA9_PIN | PA10_PIN, LL_GPIO_PIN_RESET);

  /* ### GPIO deinitialization of TIM1: TIM1_BKIN ########################### */
  ...
  /* Configure the default value IO Output Type */
  LL_GPIO_SetPinOutputType(PA6_PORT, PA6_PIN, LL_GPIO_OUTPUT_PUSHPULL);

  /* Deactivate the Pull-up and Pull-down resistor for the current IO */
  LL_GPIO_SetPinPull(PA6_PORT, PA6_PIN, LL_GPIO_PULL_NO);

  /* Reset the IO output state */
  LL_GPIO_WriteOutputPin(PA6_PORT, PA6_PIN, LL_GPIO_PIN_RESET);
}
