...
# __Example: *ll_dac_signal_generation_dma_silent_tim*__

**Example version:** 2.0.0

[![User Manual](doc/read_the-UM.svg)](https://dev.st.com/stm32cube-docs/examples/arch-v1/en/index.html "An offline version is also available in the STM32Cube firmware package.")

How to use the DAC LL API to generate a sinusoidal signal using the DMA feature in silent mode (no interrupts) with a timer trigger.
...
__Step 1__: The DAC in DMA mode and TIM are initialized by app_init().
...
__Step 3__: The DAC conversion is started using the DMA feature in silent mode and the TIM to trigger the DAC conversion.

__End of example__: The DAC conversion is repeated endlessly. Thanks to a circular buffer, each time the TIMER triggers the
DAC, the DAC requests the DMA to get a new value to convert.
The example status is reported via the variable **`ExecStatus`**,
and the status LED remains turned on in case of success.

<details>
DMA silent mode configures the DMA to operate without generating asynchronous events (interrupts).
This mode is applicable only to peripherals capable of continuous (infinite) operation under DMA control, without requiring any intervention from the Cortex.
</details>
...
__DAC__: The peripheral is configured with specific parameters:
  ...
  - Circular buffer.
  The DAC uses the DMA feature in silent mode to transfer data to be converted.

__DMA__: Configured to transfer data from the memory to the DAC each time the DAC conversion is triggered in silent mode. Data is
...
- examples_hal_dac_signal_generation_dma_tim: same example in HAL.
