...
app_status_t app_process(void)
{
  ...
  while ((com_attempts < MAX_COM_ATTEMPTS) && (return_status == EXEC_STATUS_ERROR))
  {
    ...
    /** ########## Step 2 ##########
      * Initiates the communication with the responder in a full duplex mode, using DMA, by sending and
      * receiving data simultaneously.
      * The SPI messages exchanged are null-terminated strings.
      */
    SPI_TransmitReceive_DMA((uint8_t *)pTxData, RxBuffer, BUFFER_SIZE);
