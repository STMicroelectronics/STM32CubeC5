...
int main(void)
{
  /** System Init: this generated code placed in targets folder initializes your system.
    ...
    * It also contains the LL initialization and the initial clock configuration.
    */
  ...
  else
  {
    ...
    /* Infinite execution if no error occurs */
    while (ExecStatus != EXEC_STATUS_ERROR)
    {
      ExecStatus = app_process();

      /* The system wakes up and the LED blinks once. */
      if (ExecStatus == EXEC_STATUS_OK)
      {
        LED_ON();
        LL_Delay_NoISR(EXAMPLE_LOOP_DELAY_MS);
        LED_OFF();
      }
    } /* end while */
