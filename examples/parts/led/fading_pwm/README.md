...
It illustrates it by fading an LED through a PWM.
...
__Step 1__: Initializes the MX_LED_PWM_EXAMPLE led, using the ``led_pwm_init`` LED part function.

__Step 2__: Starts fading the MX_LED_EXAMPLE led, indefinitely, using the ``led_pwm_fade` LED part function.

__End of example__: The MX_LED_EXAMPLE led fading indefinitely if no error occurs.
...
- *DMA*:
  ...
  - The DMA source and destination data widths are configured as halfword (16-bit).
