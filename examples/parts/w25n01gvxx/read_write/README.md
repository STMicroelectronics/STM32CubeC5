...
How to use the W25N01GVXX SPI NAND flash memory using erase/read/write APIs.
This example illustrates the process of writing and reading data from the SPI NAND flash memory.
...
__Step 2__: Erases the block of the memory in which we want to write.

__Step 3__: Writes a data buffer to the flash memory.

__Step 4__: Reads the same data buffer from the flash memory, through one data transfer, in continuous mode, and checks that it matches the buffer written in step 3.
...
__W25N01GVXX__: The NAND flash memory is configured with these specific parameters:

- A CS pin configured as output to be selected in the part configuration panel.
- A full feature SPI instance to be selected in the part configuration panel, as transfers can be up to 2048 bytes in length.
