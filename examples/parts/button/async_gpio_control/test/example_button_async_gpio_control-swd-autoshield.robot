...
${TEST_ID} Stress test
    [Documentation]    This test checks the robustness of the example
    ...    by pressing the button in quick sequence.
    Example Step 1
    Example Step 2
    Example Step 3 Stress
    End Of Example
...
Example Step 3 Stress
    [Documentation]    Toggle the button in quick sequence and check that the example reacts appropriately.
    ${inactive_level}=    Read Test Variable    MX_STATUS_LED_INACTIVE_STATE

    # 10 press/releases in quick sequence
    Write Pulse    BUTTON_USER_PIN    \xAA\xAA\xA0    duration=1ms

    Comment    The event count variables may be lower than expected: not all the event have been taken into account.
    ${NbButtonEvent}=    Read Integer Variable    NbButtonEvent
    Should Be True    ${NbButtonEvent} <= 20
