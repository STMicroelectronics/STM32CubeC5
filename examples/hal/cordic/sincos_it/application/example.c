/**
  ******************************************************************************
  * file           : example.c
  * brief          : How to calculate sines and cosines array in IT mode using
                     the CORDIC HAL APIs
  ******************************************************************************
  *
...
app_status_t app_process(void)
{
  ...
  /** ########## Step 3 ##########
    * Starts CORDIC calculation in IT mode.
    */
  if (HAL_CORDIC_Calculate_IT(pCORDIC, &SrcBuffer, &DstBuffer) != HAL_OK)
  {
  ...
  /** ########## Step 4 ##########
    * Waits for the CORDIC read IT transfer completion.
    * Then compares the CORDIC results to the reference values.
    */
  while (!CordicCalculationCompleted && !CordicError)
  {
    /** Put the CPU in Wait For Interrupt state. A CORDIC, IT or SystTick interrupt can wake up the CPU.
      * @user: This process is used to illustrate the interest of the interruption. It can be replaced by your own code.
      */
    __WFI();
  }
...
/** CORDIC calculation completed callback.
  ...
  * This function is executed if a CORDIC Read IT transfer is completed.
  */
