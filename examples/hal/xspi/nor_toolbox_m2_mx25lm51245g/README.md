...
# __Example: *hal_xspi_nor_toolbox_m2_mx25lm51245g*__
...
How to use the MX25LM51245G NOR flash in OctalSPI DTR and memory mapped modes, including JEDEC/SFDP identification, erase, program and read operations using the HAL API.
...
__Step 2__: reads the JEDEC ID to check that the MX25LM51245G is the expected device and correctly connected.

__Step 3__: reads memory SFDP ( Serial Flash Discoverable Parameter ) Table.

__Step 4__: configures the MX25LM51245G NOR memory in Octal-SPI DTR (double-transfer rate) mode.

__Step 5__: erases the first 4Kbyte sector of the memory, in automatic polling mode, by executing the Sector Erase command.

__Step 6__: enables, configures and starts the memory-mapped mode for octal read and write operations, so that the MX25LM51245G device is seen as an internal memory.

__Step 7__: writes the TX buffer to the first page of the MX25LM512G device.

__Step 8__: reads back the written TxBuffer from the memory first page and check data correctness.

__Step 9__: stops the memory-mapped mode.

__Step 10__: deinitializes the XSPI instance.
...
[INFO] Step 2: JEDEC ID check: OK (MX25LM51245G detected).
[INFO] Step 3: SFDP table read COMPLETED.
[INFO] Step 4: Memory configuration in DTR mode COMPLETED.
[INFO] Step 5: Memory erasing COMPLETED.
[INFO] Step 6: Memory-mapped mode STARTED.
[INFO] Step 7: Page program operation COMPLETED.
[INFO] Step 8: Memory read COMPLETED. Data written and read match. 
[INFO] Step 9: Memory-mapped mode STOPPED. 
[INFO] Step 10: de-init 
```
...
```text
@startuml
@startditaa{doc/example_xspi_nor_toolbox_m2_mx25lm51245g-setup.png} -E  -S
/-------------------------------------------\
|                    /--------------------+ |
|                    |                    | |
|                    |                    | |
...
```
-->
![example_xspi_nor_toolbox_m2_mx25lm51245g-setup](doc/example_xspi_nor_toolbox_m2_mx25lm51245g-setup.png)
