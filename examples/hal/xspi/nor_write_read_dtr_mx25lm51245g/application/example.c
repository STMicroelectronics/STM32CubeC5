/**
  ...
  *                  in automatic status-polling mode with HAL API.
  ******************************************************************************
  *
...
app_status_t app_process(void)
{
  ...
  /** ########## Step 4 ##########
    * Writes the TX buffer to the first page of the memory in automatic polling mode.
    */
  if (OctalWriteEnable() != EXEC_STATUS_OK)
  {
    goto _app_process_exit;
  ...
  if (HAL_XSPI_Transmit(pXSPI, TxBuffer, TIMEOUT) != HAL_OK)
  {
    goto _app_process_exit;
  }
  ...
  if (HAL_XSPI_Receive(pXSPI, (uint8_t *)RxBuffer, TIMEOUT) != HAL_OK)
  {
    goto _app_process_exit;
  }
