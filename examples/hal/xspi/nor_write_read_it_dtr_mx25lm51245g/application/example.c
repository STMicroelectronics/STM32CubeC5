...
volatile uint8_t StatusMatchCplt = 0U; /* Set to 1 when the automatic polling status match is reached */
...
static app_status_t OctalAutoPollMemReadyIT(void);
...
static void StatusMatchCallback(hal_xspi_handle_t *pXSPI);
...
app_status_t app_init(void)
{
  ...
  if (HAL_XSPI_RegisterStatusMatchCallback(pXSPI, StatusMatchCallback) != HAL_OK)
  {
    goto _app_init_exit;
  }
...
app_status_t app_process(void)
{
  ...
  /** ########## Step 3 ##########
    * Erases the first sector of the memory using interrupt mode.
    */
  ...
  /* Wait for the end of the command phase */
  while (CmdCplt == 0)
  {
  ...
  /* Start the automatic status check in interrupt mode to wait until the memory is ready after the Page
    * Program operation.
    */
  if (OctalAutoPollMemReadyIT() != EXEC_STATUS_OK)
  {
    goto _app_process_exit;
  }

  /* Wait for the status match event (end of erase / memory ready) */
  while (StatusMatchCplt == 0)
  {
    /** Put the CPU in Wait For Interrupt state. A XSPI interrupt or a SystTick interrupt can wake up the CPU.
      * @user: This process is used to illustrate the interest of the interrupts. It can be replaced by your own code.
      */
  ...
  /** ########## Step 4 ##########
    * Writes the TX buffer to the first page of the memory in IT mode.
    */
  if (OctalWriteEnable() != EXEC_STATUS_OK)
  {
    goto _app_process_exit;
  ...
  /* Transmit the TX buffer in interrupt mode */
  if (HAL_XSPI_Transmit_IT(pXSPI, TxBuffer) != HAL_OK)
  {
    goto _app_process_exit;
  }

  /* Wait for the end of the data phase */
  ...
  /* Wait for the status match event (memory ready) */
  while (StatusMatchCplt == 0)
  {
    /** Put the CPU in Wait For Interrupt state. A XSPI interrupt or a SystTick interrupt can wake up the CPU.
      * @user: This process is used to illustrate the interest of the interrupts. It can be replaced by your own code.
  ...
  /** ########## Step 5 ##########
    * Reads back the written TxBuffer from the memory.
    */
  ...
  /* Send the Read command in interrupt mode */
  if (HAL_XSPI_SendRegularCmd_IT(pXSPI, &command) != HAL_OK)
  {
    goto _app_process_exit;
  }
  ...
  if (HAL_XSPI_Receive_IT(pXSPI, (uint8_t *)RxBuffer) != HAL_OK)
  {
    goto _app_process_exit;
  }

  /* Wait for the end of the receive phase */
...
/** brief:  Starts an automatic polling sequence in interrupt mode to wait until the memory is ready.
  * The function configures a Read Status Register command and the HAL automatic polling mechanism,
  * then launches it in interrupt mode.
  * When the match condition is met (end of program/erase/write operation),
  * the XSPI status match callback is called.
  * param:  None
  * retval: Application status
  */
static app_status_t OctalAutoPollMemReadyIT(void)
{
  ...
  if (HAL_XSPI_SendRegularCmd(pXSPI, &command, TIMEOUT) != HAL_OK)
  {
    goto _OctalAutoPollMemReadyIT_exit;
  }
  ...
  polling_mode_cfg.interval_cycle = MX25_AUTO_POLLING_INTERVAL;
  polling_mode_cfg.automatic_stop_status = HAL_XSPI_AUTOMATIC_STOP_ENABLED;
  if (HAL_XSPI_ExecRegularAutoPoll_IT(pXSPI, &polling_mode_cfg) != HAL_OK)
  {
    goto _OctalAutoPollMemReadyIT_exit;
  }
...
/** XSPI status match callback.
  * @user: This implementation of the XSPI status match callback can be customized.
  */
static void StatusMatchCallback(hal_xspi_handle_t *pXSPI)
{
