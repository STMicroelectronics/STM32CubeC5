...
app_status_t app_init(void)
{
  ...
  HAL_UART_EnableAutoBaudRate(pUART);
...
app_status_t app_process(void)
{
  ...
  while ((com_attempts < MAX_COM_ATTEMPTS) && (return_status == EXEC_STATUS_ERROR))
  {
    ...
    hal_status = HAL_UART_Receive(pUART, RxBuffer, BUFFER_SIZE, TIMEOUT);
    ...
    /** ########## Step 3 ##########
      * Verifies that the auto baud rate detection status completed successfully.
      */
    if (HAL_UART_GetAutoBaudRateStatus(pUART) == HAL_UART_AUTO_BAUD_RATE_DET_SUCCESS)
    {
    ...
    hal_status = HAL_UART_Transmit(pUART, RxBuffer, BUFFER_SIZE, TIMEOUT);
