...
static const uint8_t TxStartMessage[] = "\r\n\nDMA-Please use the input line below to send data to the device\n\r\
[input]<<";
...
app_status_t app_process(void)
{
  ...
  /** ########## Step 2 ##########
    * Transmits in dma mode a start message using UART.
  */
  hal_status = HAL_UART_Transmit_DMA(pUART, TxStartMessage, sizeof(TxStartMessage) - 1);
  ...
  */
  ...
  /** ########## Step 4 ##########
    * Start the reception in dma mode using UART.
  */
  ...
  /** The detection of new line ('\n') character or the amount of data elements is received marks the end
    * of the reception.
    */
  hal_status =  HAL_UART_ReceiveUntilCM_DMA(pUART, DataBuffer, BUFFER_SIZE, '\n');
  if (hal_status != HAL_OK)
  {
  ...
  */
  ...
  hal_status = HAL_UART_Transmit_DMA(pUART, DataBuffer, BUFFER_SIZE);
  if (hal_status != HAL_OK)
  {
    /* An error occurs at the startup of the UART transmission. */
    return return_status;
  ...
  */
  ...
  {
    /* The full RX-TX cycle is completed successfully */
