...
*** Settings ***
Documentation       Automatic test for example_hal_spi_full_duplex_two_boards_com_polling_responder.
...
*** Keywords ***
Example Step 1
    [Documentation]    Checks the configuration and initialization of the UART instance.
    # Check Integer Variable    ExecStatus    ${EXEC_STATUS_INIT_OK}
    Read Console Until    [INFO] Step 1: Device initialization COMPLETED.

Example Step 2
    [Documentation]    initiates the communication with the controller in a full duplex mode, in polling mode,
