/**
  ******************************************************************************
  * file           : example.c
  * brief          : A full-duplex synchronous SPI communication with CRC as controller,
  *                  in interrupt mode, using the HAL API.
  ******************************************************************************
  *
...
app_status_t app_process(void)
{
  ...
  while ((com_attempts < MAX_COM_ATTEMPTS) && (return_status == EXEC_STATUS_ERROR))
  {
    ...
    /** ########## Step 2 ##########
      * Initiates the communication with the responder in a full duplex mode, using interrupts, by sending and
...
/** brief:  This function is executed in case of a data transfer error, including CRC errors.
