...
# __Example: *hal_spi_half_duplex_two_boards_com_polling_controller*__

**Example version:** 2.0.0

[![User Manual](doc/read_the-UM.svg)](https://dev.st.com/stm32cube-docs/examples/arch-v1/en/index.html "An offline version is also available in the STM32Cube firmware package.")

How to manage a half duplex synchronous SPI communication as controller, in polling mode, using the HAL API.
...
__Step 2__: initiates communication in half-duplex blocking mode. The controller transmits data to the responder within a specific timeout period. A counter of attempts is reset when initiating the communication loop.

__Step 3__: the responder receives data from the controller within a specific timeout period. A counter of attempts is reset when initiating the communication loop.
...
__End of example__: If no error occurs, the data is transferred indefinitely from the controller to the responder. If the maximum number of attempts is reached, the data transfer is stopped and an error status is reported to the `main()` function.

If the data transmit operation fails or the exchanged buffers are different, the controller restarts the communication by sending again the same message. The `error_handler()` function is called when the maximum number of attempts is reached.

If you enable **`USE_TRACE`**, you can follow these steps, in the nominal case of execution, in the terminal logs:

```text
[INFO] Step 1: Device initialization COMPLETED.
[INFO] Controller - Tx/Rx Buffers IDENTICAL. Transfer COMPLETED of SPI Half Duplex Two Boards Communication - Message A
[INFO] Controller - Tx/Rx Buffers IDENTICAL. Transfer COMPLETED of SPI Half Duplex Two Boards Communication - Message B
[INFO] Controller - Tx/Rx Buffers IDENTICAL. Transfer COMPLETED of SPI Half Duplex Two Boards Communication - Message A
[INFO] Controller - Tx/Rx Buffers IDENTICAL. Transfer COMPLETED of SPI Half Duplex Two Boards Communication - Message B
```

  > **_NOTE:_** In this example a delay of 50ms is used between each transmission. It is an arbitrary delay to ensure synchronization between the controller and the responder. It depends on the controller and responder application and can be modified by the user.
...
![spi-half-duplex-two-boards-com-polling-controller_NominalCase](doc/spi-half-duplex-two-boards-com-polling-controller_NominalCase.svg)

<details>
<summary> Expand this tab to visualize the sequence chart diagram of the communication attempts' mechanism. </summary>

![spi-half-duplex-two-boards-com-polling-controller_AttemptsMechanism](doc/spi-half-duplex-two-boards-com-polling-controller_AttemptsMechanism.svg)
...
The example uses SPI half-duplex synchronous transfers on two lines (MOSI and the clock signal) to exchange data from the controller to the responder. For this purpose, the SPI instance of the controller board should be configured to the 'Master' mode, and the direction should be set to one line.
...
__Pins alignment__: When connecting the pins of the controller board to the ones of the responder board, the MISO line of the controller is connected to the MOSI line of the responder, and the same goes for the SCK line.
...
__Transmission synchronization__: In half-duplex mode, the controller cannot check the status of the responder. To be sure the responder is ready to send or receive data from the controller, additional delay can be necessary to handle this synchronization.
...
- hal_spi_half_duplex_two_boards_com_polling_responder: half duplex synchronous SPI communication as responder, in polling mode.
