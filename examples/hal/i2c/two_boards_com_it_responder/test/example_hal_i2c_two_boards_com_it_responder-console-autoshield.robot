...
*** Settings ***
Documentation       Automatic test for example_hal_i2c_two_boards_com_it_responder.
...
*** Test Cases ***
${TEST_ID} Debug Test - Check I2C addresses
    [Documentation]    This test is designed to check if expected I2C adresses are active on the bus (target and/or external sensors).

    Init I2c    I2C_SCL    I2C_SDA    ${CONTROLLER_ADDRESS}    baudrate=${BAUDRATE}    with_pullups=${True}
    @{addresses}=    Get I2C Addresses
    Should Be True    "${TARGET_ADDRESS}" in ${addresses}
