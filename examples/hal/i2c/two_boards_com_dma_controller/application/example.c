/**
  ...
  *                  I2C in a non-blocking mode using DMA and HAL API.
  ******************************************************************************
  *
...
app_status_t app_process(void)
{
  ...
  while ((com_attempts < MAX_COM_ATTEMPTS) && (return_status == EXEC_STATUS_ERROR))
  {
    ...
    /** ########## Step 2 ##########
      * Transmits in DMA mode the buffer pointed by pTxData, containing a null-terminated string, using I2C.
      */
    hal_status = HAL_I2C_MASTER_Transmit_DMA(pI2C, I2C_RESPONDER_ADDR_7BIT_SHIFTED, pTxData, BUFFER_SIZE);
    ...
    /** ########## Step 4 ##########
      * Receives in DMA mode a buffer containing a null-terminated string using I2C.
      */
    hal_status = HAL_I2C_MASTER_Receive_DMA(pI2C, I2C_RESPONDER_ADDR_7BIT_SHIFTED, RxBuffer, BUFFER_SIZE);
    if (hal_status != HAL_OK)
    {
