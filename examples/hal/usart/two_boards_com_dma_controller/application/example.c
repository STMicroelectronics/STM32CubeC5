/**
  ******************************************************************************
  * file           : example.c
  * brief          : The USART controller communication side with DMA
  *                  using HAL API.
  ******************************************************************************
  *
...
app_status_t app_process(void)
{
  ...
  while ((com_attempts < MAX_COM_ATTEMPTS) && (return_status == EXEC_STATUS_ERROR))
  {
    ...
    /** ########## Step 2 ##########
      * Transmits in DMA mode a buffer pointed by pTxData using USART.
      */
    hal_status = HAL_USART_Transmit_DMA(pUSART, pTxData, BUFFER_SIZE);
    ...
    /** ########## Step 4 ##########
      * Receives in DMA mode a buffer using USART.
      */
