/**
  ******************************************************************************
  * file           : example.c
  * brief          : The USART responder communication side with DMA
  *                  using HAL API.
  ******************************************************************************
  *
...
app_status_t app_process(void)
{
  ...
  while ((com_attempts < MAX_COM_ATTEMPTS) && (return_status == EXEC_STATUS_ERROR))
  {
    ...
    /** ########## Step 2 ##########
      * Receives in DMA mode a buffer containing a null-terminated string using USART.
      */
    hal_status = HAL_USART_Receive_DMA(pUSART, RxBuffer, BUFFER_SIZE);
    ...
    /** ########## Step 4 ##########
      * Transmits in DMA mode a buffer using USART.
      */

    hal_status = HAL_USART_Transmit_DMA(pUSART, RxBuffer, BUFFER_SIZE);
