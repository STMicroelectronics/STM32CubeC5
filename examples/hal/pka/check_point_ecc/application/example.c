...
app_status_t app_process(void)
{
  ...
  if (HAL_PKA_SetConfigMontgomery(pPKA, &MontgomeryConfig) != HAL_OK)
  {
  ...
  /** ########## Step 3 ##########
      ...
      */
  /* Initialize the PKA configuration parameters */
  ...
  /* Compute point P(x,y) on the curve primv256V1 */
  if (HAL_PKA_Compute(pPKA, PKA_TIMEOUT) != HAL_OK)
  {
    goto _app_process_exit;
  }

  PRINTF("[INFO] Step 3.2: Computation COMPLETED.\n");
  ...
  PRINTF("[INFO] Step 3.5: Computation COMPLETED.\n");
  /* Verifies if the calculated point is on the curve */
