...
hal_uart_handle_t *mx_usart2_uart_init(void)
{
  ...
  if (HAL_RCC_USART2_SetKernelClkSource(HAL_RCC_USART2_CLK_SRC_HSIK) != HAL_OK)
  {
    return NULL;
  }
  ...
  if (HAL_UART_DisableRxOverRunDetection(&hUSART2) != HAL_OK)
  {
    return NULL;
  }
  ...
  HAL_GPIO_Init(DBGIN_VCP_TX_PORT, DBGIN_VCP_TX_PIN, &gpio_config);
...
(void)HAL_UART_DeInit(&hUSART2);
  ...
  HAL_GPIO_DeInit(DBGIN_VCP_TX_PORT, DBGIN_VCP_TX_PIN);
}
