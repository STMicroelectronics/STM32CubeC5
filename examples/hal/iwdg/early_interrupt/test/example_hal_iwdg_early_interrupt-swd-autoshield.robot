...
Example Step 4
    [Documentation]    The flash/nvm memory is programmed during the interrupt.
    Comment    Nothing to verify
...
Example Step 5
    ...
    ...    After 1s, the watchdog fires and resets the example.
    ...
    # Here the example takes around 500ms*3 + 3000ms = 4500ms. As the access with CubeProgrammer might be a bit long, we set the timeout to 6s.
    ${active_level}=    Read Test Variable    MX_STATUS_LED_ACTIVE_STATE
    Check Pin    MX_STATUS_LED    ${active_level}    timeout=6s
    Check Pin    MX_STATUS_LED    ${active_level}    duration=1s
    Check Integer Variable    ExecStatus    ${EXEC_STATUS_OK}    timeout=1s

    Comment    Once the device has restarted, checks the data have been correctly written in the memory during the
    ...    interrupt.
