...
*** Settings ***
Documentation       Automatic test for example_hal_dac_signal_generation_dma_silent_tim.
...
Example Step 2
    ...
    ${embedded_buffer}=    Read Raw Variable    SineWave    size= ${SINE_BUFFER_SIZE_BYTE}

Example Step 3
    [Documentation]    The DAC conversion is started using the DMA feature and the TIM started to trigger the DAC
    ...    conversion.
    Check Frequency    DAC_PIN    ${SIGNAL_FREQ}    0.05

    ${converted_buffer}=    Read Test Variable    CONVERTED_BUFFER
    Check Signal    DAC_PIN    ${SIGNAL_FREQ}    ${converted_buffer}    0.03
