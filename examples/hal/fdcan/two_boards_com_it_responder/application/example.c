...
{
  app_status_t return_status = EXEC_STATUS_ERROR;
  hal_status_t hal_status = HAL_OK;   /* memorizes the HAL status of the FDCAN TX/RX operations */
  ...
  hal_fdcan_tx_header_t tx_element_header =
  {
    ...
    .b.data_length = HAL_FDCAN_DATA_LEN_CAN_FDCAN_0_BYTE,  /* This field will be updated in next steps */
  };
  ...
  while ((com_attempts < MAX_COM_ATTEMPTS) && (return_status == EXEC_STATUS_ERROR))
  {
    ...
    /** ########## Step 2 ##########
      * Receives in interrupt mode a buffer using FDCAN.
      */
    ...
    /** ########## Step 3 ##########
      * Transmit in interrupt mode the previous FDCAN frame (DATA and DLC).
      */
