...
__Step 2__: starts the communication by sending a message to the responder in interrupt mode, within a specific timeout period. A counter of attempts is reset when initiating the communication loop.

__Step 3__: expects to receive the filtered message back within the specified timeout period, in interrupt mode.

__Step 4__: checks that the sent and received messages in RX_FIFO_0 match (DLC, and data fields) and continues to __Step2__ if no error occurs.
...
![fdcan-two-boards-com-interrupt_RetryMechanism-TxError](doc/fdcan-two-boards-com-interrupt_RetryMechanism-TxError.svg)
</details>
...
- The FDCAN filter is configured to:
  - store FDCAN_RESPONDER_FRAME_ID frame in RX_FIFO_0 and reject all other standard frames.
...
<!--
@startditaa{doc/example_hal_fdcan_2b_com_interrupt_controller-setup.png} -E -S
    /-------------------------\                                                /-------------------------\
    |          /--------------+       FDCAN                        FDCAN       +--------------\          |
...
![example_hal_fdcan_two_boards_com_interrupt_controller-setup](doc/example_hal_fdcan_2b_com_interrupt_controller-setup.png)
