...
*** Settings ***
Documentation       Automatic test for example_hal_fdcan_two_boards_com_polling_controller.
...
${TEST_ID} Retry mechanisms
    ...
    Comment    Receive message but reply only after few attempts
    FOR    ${i}    IN RANGE    ${RETRY_ATTEMPTS}
        Example Step 2    ${MESSAGE_PING}
        Sleep    ${CONTROLLER_RETRY_DELAY}
    END
    Example Step 2    ${MESSAGE_PING}
    Example Step 3    ${MESSAGE_PING}
    Example Step 4

    Comment    Receive message but never answer
    FOR    ${i}    IN RANGE    ${MAX_ATTEMPTS}
        Example Step 2    ${MESSAGE_PONG}
        Sleep    ${CONTROLLER_RETRY_DELAY}
    END
...
Example Step 2
    [Documentation]    The controller transmits in polling mode a FDCAN message with the correct ID.
...
Example Step 3
    [Documentation]    The controller expects to receive the FDCAN message back in polling mode with correct ID.
...
Check Failed Status
    ...
    Check Integer Variable    ExecStatus    ${EXEC_STATUS_ERROR}
    ${inactive_level}=    Read Test Variable    MX_STATUS_LED_INACTIVE_STATE
    Check Pin    MX_STATUS_LED    ${LOW}    timeout=200ms
