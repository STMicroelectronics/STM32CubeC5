...
system_status_t mx_rtc_init(void)
{
  ...
  calendar_date.mon = HAL_RTC_MONTH_JANUARY;
  calendar_date.mday = 1U;
  calendar_date.year = 0U;

  hal_rtc_time_t calendar_time;
  calendar_time.am_pm = HAL_RTC_TIME_FORMAT_AM_24H;
  calendar_time.hour = 0U;
  ...
  wakeup_config.clock = HAL_RTC_WAKEUP_TIMER_CLOCK_RTCCLK_DIV8;
  if (HAL_RTC_WAKEUP_SetConfig(&wakeup_config) != HAL_OK)
  {
    return SYSTEM_PERIPHERAL_ERROR;
  }

  uint32_t wakeup_reload_time = 12288U;
  uint32_t wakeup_clear_time = 12288U;
