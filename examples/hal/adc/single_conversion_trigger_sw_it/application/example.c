...
volatile uint8_t AdcConversionComplete = 0U;  /* Set to 1 if the ADC conversion is correctly completed */
...
static void AdcConvCpltCallback(hal_adc_handle_t *pADC);
...
app_status_t app_init(void)
{
  ...
  if (HAL_ADC_RegisterRegUnitaryConvCpltCallback(pADC, AdcConvCpltCallback) != HAL_OK)
  {
    goto _app_init_exit;
  }
...
app_status_t app_process(void)
{
  ...
  /** Awaiting the Step 4 ADC completion (or error): asynchronous ADC interrupt callback.
    ...
    */
  while ((AdcConversionComplete != 1U) && (AdcError != 1U))
  {
...
/** ########## Step 4 ##########
  ...
  */
static void AdcConvCpltCallback(hal_adc_handle_t *hADC)
{
  ...
  /* Update the status variable of the ADC unitary conversion */
