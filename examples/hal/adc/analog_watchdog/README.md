...
# __Example: *hal_adc_analog_watchdog*__

**Example version:** 2.0.0

[![User Manual](doc/read_the-UM.svg)](https://dev.st.com/stm32cube-docs/examples/arch-v1/en/index.html "An offline version is also available in the STM32Cube firmware package.")

How to use the ADC with an ADC analog watchdog to monitor a channel
and detect when the corresponding conversion data is outside the window thresholds. This example is based on the ADC HAL API.
...
__Step 1__:
The ADC is configured to convert a single channel, in continuous conversion mode, from software trigger.
Analog watchdog is configured to monitor all channels on ADC group regular (therefore, including the selected channel).
            Registers the user callbacks for ADC events: conversion completed and error.
...
__Step 3__: The ADC conversion is started in interrupt mode: ADC interruption enabled with analog watchdog.

__Step 4__: The ADC waits for conversion data is out of the analog watchdog window. When the analog watchdog callback is triggered,
 retrieve the ADC raw value and compute the corresponding voltage. Prints the ADC raw value and the computed corresponding voltage.
 Then returns to __Step 3__ if no error occurs.

__End of example__: The analog watchdog flag is checked every second endlessly (step 4 is running in a loop)
...
[INFO] Step 3: ADC start  analog watchdog 1 interruption.
[INFO] Step 4: Conversion data is in  analog watchdog window
[INFO] Step 3: ADC start  analog watchdog 1 interruption.
[INFO] Step 4: Conversion data is out of analog watchdog window.  Raw ADC value = 0x1c0   Voltage = 361 mV
...
```
...
- Analog watchdog thresholds: The high threshold is set to 1.65V. The low threshold is set to 0V.
