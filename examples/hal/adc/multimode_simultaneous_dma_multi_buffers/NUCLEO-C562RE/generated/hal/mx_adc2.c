...
/******************************************************************************/
/* Exported functions for ADC2 in HAL layer (SW instance MyADC_1) */
/******************************************************************************/
hal_adc_handle_t *mx_adc2_cfg1_hal_init(void)
{
  ...
  HAL_GPIO_Init(HAL_GPIOC, HAL_GPIO_PIN_5, &config_gpio);
  ...
  HAL_ADC_SetConfigChannel(&hADC2, HAL_ADC_CHANNEL_5, &adc_channel_config);
  ...
  if (HAL_DMA_Init(&hGPDMA1_CH1, HAL_LPDMA1_CH1) != HAL_OK)
  {
    return NULL;
  }

  hal_dma_direct_xfer_config_t xfer_cfg_adc2_dma;

  xfer_cfg_adc2_dma.request         = HAL_LPDMA1_REQUEST_ADC2;
  xfer_cfg_adc2_dma.direction       = HAL_DMA_DIRECTION_PERIPH_TO_MEMORY;
  xfer_cfg_adc2_dma.src_inc         = HAL_DMA_SRC_ADDR_FIXED;
  xfer_cfg_adc2_dma.dest_inc        = HAL_DMA_DEST_ADDR_INCREMENTED;
  xfer_cfg_adc2_dma.src_data_width  = HAL_DMA_SRC_DATA_WIDTH_WORD;
  xfer_cfg_adc2_dma.dest_data_width = HAL_DMA_DEST_DATA_WIDTH_WORD;
  xfer_cfg_adc2_dma.priority        = HAL_DMA_PRIORITY_HIGH;
  if (HAL_DMA_SetConfigDirectXfer(&hGPDMA1_CH1, &xfer_cfg_adc2_dma) != HAL_OK)
  {
    return NULL;
  }
  ...
  HAL_ADC_REG_SetDMA(&hADC2, &hGPDMA1_CH1);

  /* Set ADC global interruption */
  HAL_CORTEX_NVIC_SetPriority(ADC2_IRQn, HAL_CORTEX_NVIC_PREEMP_PRIORITY_0, HAL_CORTEX_NVIC_SUB_PRIORITY_0);
  HAL_CORTEX_NVIC_EnableIRQ(ADC2_IRQn);
