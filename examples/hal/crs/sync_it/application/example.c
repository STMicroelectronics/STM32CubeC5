...
volatile uint8_t CRS_sync_status = 0U;  /* variable to check the synchronisation status */

/* Private functions prototype -----------------------------------------------*/
/* Function allowing the user to configure dynamically the CRS callbacks instead of weak functions */
static void CrsSyncOkCallback(hal_crs_handle_t *hcrs);
/** ########## Step 1 ##########
...
app_status_t app_init(void)
{
  ...
  /* Register the CRS callback */
  HAL_CRS_RegisterSyncOkCallback(pCRS, CrsSyncOkCallback);
...
app_status_t app_process(void)
{
  ...
  /** ########## Step 2 ##########
    * Starts the CRS synchronization
    */
  if (HAL_CRS_StartSync_IT(pCRS) != HAL_OK)
  {
  ...
  /** ########## Step 3 ##########
    * Checks the synchronization is finished
    */
  while (CRS_sync_status != 1U)
  {
