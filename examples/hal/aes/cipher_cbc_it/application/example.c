/**
  ...
  *                  and decryption in interrupt mode using the AES CBC algorithm.
  *
  ******************************************************************************
  *
...
/** ########## Step 1 ##########
  * Initializes, configures the AES instance and registers callbacks.
...
app_status_t app_init(void)
{
  ...
  PRINTF("[INFO] Step 1.1: AES peripheral initialization and configuration to work with CBC mode COMPLETED.\n");
...
/** ########## Step 2 ##########
  * Encrypts in interrupt mode a known plaintext and verifies the generated ciphertext.
  * Then decrypts in interrupt mode a known ciphertext and verifies the generated plaintext.
  * This step is scheduled as follows:
  * Step 2.1: Encrypts in interrupt mode a known plaintext then verifies the generated ciphertext.
  * Step 2.2: Configures the AES peripheral with the CBC algorithm with decryption method.
  * Step 2.3: Decrypts in interrupt mode a known ciphertext then verifies the generated plaintext.
...
app_status_t app_process(void)
{
  ...
  /** ########## Step 2.1 ##########
    * Encrypts in interrupt mode a known plaintext then verifies the generated ciphertext.
    */
  hal_status = HAL_AES_Encrypt_IT(pAES,                /* Pointer to a hal_aes_handle_t structure */
  ...
  OutTransferCpltCb = 0;
  ErrorCb = 0;
  PRINTF("[INFO] Step 2.1: Encryption in interrupt mode and verification COMPLETED.\n");
  ...
  PRINTF("[INFO] Step 2.2: Configuration of the AES peripheral to work with CBC in decryption mode COMPLETED.\n");

  /** ########## Step 2.3 ##########
    * Decrypts in interrupt mode of a known ciphertext, followed by the verification of the generated plaintext.
    */
  hal_status = HAL_AES_Decrypt_IT(pAES,                 /* Pointer to a hal_aes_handle_t structure */
  ...
  OutTransferCpltCb = 0;
  ErrorCb = 0;
  PRINTF("[INFO] Step 2.3: Decryption in interrupt mode and verification COMPLETED.\n");
