...
hal_aes_handle_t *mx_aes_init(void)
{
  ...
  HAL_CORTEX_NVIC_SetPriority(AES_IRQn, HAL_CORTEX_NVIC_PREEMP_PRIORITY_0, HAL_CORTEX_NVIC_SUB_PRIORITY_0);
  HAL_CORTEX_NVIC_EnableIRQ(AES_IRQn);
  return &hAES;
}

void mx_aes_deinit(void)
{
  /* Disable the interruption for AES */
  HAL_CORTEX_NVIC_DisableIRQ(AES_IRQn);
