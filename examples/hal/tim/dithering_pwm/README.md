...
Note: Depending on the board, the on-board User LED may not be connected to the selected timer PWM output. In this case, connect an external LED (with an appropriate series resistor) to the configured PWM output pin (CHANNEL_Z) to demonstrate the PWM output (or use an oscilloscope/logic analyzer to observe the signal).
