...
# __Example: *hal_tim_combined_pwm*__

**Example version:** 2.0.0

[![User Manual](doc/read_the-UM.svg)](https://dev.st.com/stm32cube-docs/examples/arch-v1/en/index.html "An offline version is also available in the STM32Cube firmware package.")

How to configure a TIM peripheral to generate combined 3-phase PWM on 3 channels (say 'x', 'y', and 'z') with a signal reference on the channel 5.
...
__Step 2__: Starts the timer PWM generation for all channels.
...
- The timer channels (say 'x', 'y', and 'z') are configured to generate PWM signals in center aligned up down counting mode.
- The timer prescaler is configured to set the timer counter clock to 1 MHz.
- The PWM duty cycle is configured at 50% for channel x, 37.5% for channel y and 25% for channel z.
- Channel 5 is used in PWM2 mode with a duty cycle set to 6.22%.
- The 3 resulting signals are made of an AND logical combination of two reference PWMs:
        Channel x and Channel 5
        Channel y and Channel 5
        Channel z and Channel 5
The TIM1 waveform can be displayed using an oscilloscope.

![combined_pwm](doc/combined_pwm_signal.svg)
...
<details>
  ...
  To set a PWM output frequency to 24kHz with a 1MHz timer counter clock:
   ...
   To set the channel x's PWM duty cycle to 50%:
    ...
    CCRx = (50 / 100) * 41 = 20.5
    CCRx = 20 (integer rounded down to fit into the register)

   To set the channel y's PWM duty cycle to 37.5%:

    CCRy = (37.5 / 100) * 41 = 15.375
    CCRy = 15 (integer rounded down to fit into the register)

   To set the channel z's PWM duty cycle to 25%:

    CCRz = (25 / 100) * 41 = 10.25
    CCRz = 10 (integer rounded down to fit into the register)

    To set the channel q's PWM duty cycle to 6.22%:

    CCRz = (6.22 / 100) * 41 = 2.55
    CCRz = 2 (integer rounded down to fit into the register)
...
- The channel x PWM output duty cycle is _20 / 41 = 48.78%_
- The channel y PWM output duty cycle is _15 / 41 = 36.58%_
- The channel z PWM output duty cycle is _10 / 41 = 24.39%_
- The channel q PWM output duty cycle is _2 / 41 = 6.22%_
...
- hal_tim_pwm_output: demonstrates how to use the TIM peripheral to measure the frequency and duty cycle limits of a signal.
