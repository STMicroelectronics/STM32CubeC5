...
*** Settings ***
Documentation       Automatic test for example_hal_tim_asymmetric_deadtime_insertion.
...
*** Variables ***
${DEADTIME_DELAY_FALLING}=      0.0000037
${DEADTIME_DELAY_RISING}=       0.0000063
...
Example Step 2
    ...
    Comment    Set t_min and t_max with 2 microseconds margin over DEADTIME_DELAY_RISING.
    ${MIN_DELAY_RISING_MS}=    Set Variable    ${{ ${DEADTIME_DELAY_RISING} - ${DEADTIME_DELAY_MARGIN} }}
    ${MAX_DELAY_RISING_MS}=    Set Variable    ${{ ${DEADTIME_DELAY_RISING} + ${DEADTIME_DELAY_MARGIN} }}
    Comment    Set t_min and t_max with 2 microseconds margin over DEADTIME_DELAY_FALLING.
    ${MIN_DELAY_FALLING_MS}=    Set Variable    ${{ ${DEADTIME_DELAY_FALLING} - ${DEADTIME_DELAY_MARGIN} }}
    ${MAX_DELAY_FALLING_MS}=    Set Variable    ${{ ${DEADTIME_DELAY_FALLING} + ${DEADTIME_DELAY_MARGIN} }}

    Comment    Check output PWM frequency and duty cycle for channel CHy
    Check Frequency    pin=MX_EXAMPLE_USER_PWM_CHy_PIN    freq=8000Hz    duty_cycle=0.45    precision=0.1
    ...
    ...    t_min= ${MIN_DELAY_FALLING_MS}
    ...    t_max=${MAX_DELAY_FALLING_MS}
    ...
    ...    evt2_edge=rising edge
    ...    t_min= ${MIN_DELAY_RISING_MS}
    ...    t_max=${MAX_DELAY_RISING_MS}
    ...    time_window=${TIME_WINDOW}
