...
hal_tim_handle_t *mx_tim1_init(void)
{
  ...
  if (HAL_TIM_SetDeadtime(&hTIM1, 249, 226) != HAL_OK)
  {
    return NULL;
  }
  if (HAL_TIM_EnableAsymmetricalDeadtime(&hTIM1) != HAL_OK)
  {
    return NULL;
  }
  ...
  HAL_GPIO_Init(HAL_GPIOA, PA8_PIN | PA7_PIN, &gpio_config);
...
void mx_tim1_deinit(void)
{
  ...
  HAL_GPIO_DeInit(HAL_GPIOA, PA7_PIN | PA8_PIN);
}
