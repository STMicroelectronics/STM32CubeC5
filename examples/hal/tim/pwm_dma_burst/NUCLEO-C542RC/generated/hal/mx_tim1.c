...
hal_tim_handle_t *mx_tim1_init(void)
{
  ...
  /* Timer configuration to reach the output frequency at 122 Hz */
  ...
  oc_compare_unit_config.pulse = 0x8000;
  ...
  if (HAL_TIM_OC_EnableComparePreload(&hTIM1, HAL_TIM_OC_COMPARE_UNIT_2) != HAL_OK)
  {
    return NULL;
  }
  ...
  if (HAL_TIM_OC_EnableComparePreload(&hTIM1, HAL_TIM_OC_COMPARE_UNIT_3) != HAL_OK)
  {
    return NULL;
  }
  ...
  oc_compare_unit_config.mode  = HAL_TIM_OC_PWM1;
  oc_compare_unit_config.pulse = 0x8000;
  if (HAL_TIM_OC_SetConfigCompareUnit(&hTIM1, hal_tim_oc_channel_to_compare_unit(HAL_TIM_CHANNEL_4),
                                      &oc_compare_unit_config) != HAL_OK)
  {
    return NULL;
  }

  if (HAL_TIM_OC_EnableComparePreload(&hTIM1, HAL_TIM_OC_COMPARE_UNIT_4) != HAL_OK)
  {
    return NULL;
  }
  ...
  dmaburst.address = HAL_TIM_DMABURST_BASE_ADDR_CCR2;
  dmaburst.source = HAL_TIM_DMABURST_UPD;
  dmaburst.length = HAL_TIM_DMABURST_3TRANSFERS;
  if (HAL_TIM_SetConfigDMABurst(&hTIM1, &dmaburst) != HAL_OK)
  {
    return NULL;
  }
  ...
  HAL_GPIO_Init(HAL_GPIOA, HAL_GPIO_PIN_9 | HAL_GPIO_PIN_10 | HAL_GPIO_PIN_11, &gpio_config);
...
hal_dma_direct_xfer_config_t xfer_cfg_tim1_upd_dma;
  xfer_cfg_tim1_upd_dma.request         = HAL_LPDMA1_REQUEST_TIM1_UPD;
  xfer_cfg_tim1_upd_dma.direction       = HAL_DMA_DIRECTION_MEMORY_TO_PERIPH;
  xfer_cfg_tim1_upd_dma.src_inc         = HAL_DMA_SRC_ADDR_INCREMENTED;
  xfer_cfg_tim1_upd_dma.dest_inc        = HAL_DMA_DEST_ADDR_FIXED;
  xfer_cfg_tim1_upd_dma.src_data_width  = HAL_DMA_SRC_DATA_WIDTH_WORD;
  xfer_cfg_tim1_upd_dma.dest_data_width = HAL_DMA_DEST_DATA_WIDTH_WORD;
  xfer_cfg_tim1_upd_dma.priority        = HAL_DMA_PRIORITY_HIGH;

  if (HAL_DMA_SetConfigPeriphDirectXfer(&hLPDMA1_CH0, &xfer_cfg_tim1_upd_dma) != HAL_OK)
  {
    return NULL;
  }
  ...
  if (HAL_TIM_SetDMA(&hTIM1, HAL_TIM_DMA_ID_UPD, &hLPDMA1_CH0) != HAL_OK)
  {
    return NULL;
  }
...
void mx_tim1_deinit(void)
{
  ...
  HAL_GPIO_DeInit(HAL_GPIOA, HAL_GPIO_PIN_9 | HAL_GPIO_PIN_10 | HAL_GPIO_PIN_11);
