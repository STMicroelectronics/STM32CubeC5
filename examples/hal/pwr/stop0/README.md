...
# __Example: *hal_pwr_stop0*__

**Example version:** 2.0.0

[![User Manual](doc/read_the-UM.svg)](https://dev.st.com/stm32cube-docs/examples/arch-v1/en/index.html "An offline version is also available in the STM32Cube firmware package.")

How to enter and exit the Stop0 mode through an EXTI interrupt using the HAL API.


## __1. Detailed scenario__

We illustrate this by switching the MCU from the Run to Stop0 mode and waking the MCU up using an EXTI interrupt.

__Initialization phase__: At main program start, the `mx_system_init()` function is called. It initializes the peripherals, nonvolatile memory (such as flash memory, NVM, or external memories), MPU regions (if applicable), the system clock, and the SysTick.

The application executes the following __example steps__:

__Step 1__: At startup, the application configures the system to reach lowest power consumption while the system is in Stop0 mode and configures the system wake-up source.

__Step 2__: The example stays in RUN mode with LED on during 2s, then clears the pending flags before entering Stop0 mode. At this point, the device power consumption can be measured until the user push button connected to the EXTI line is pressed to wake up the system.
...
The Cortex supports two power states: Sleep and Deep-sleep. Stop0 mode is a deep-sleep mode of the MCU, so it is entered with the Cortex in Deep-sleep state.

In Stop0 mode, the Cortex and most clocks are stopped, while SRAM and register contents are retained.

Exiting Stop0 mode requires configuring a wake-up source. A GPIO in EXTI interrupt mode is configured within this example.

Exiting Stop0 mode does not reset the MCU. After wake-up, execution resumes where it stopped, and the system clock must be reconfigured. The STOP flag should be cleared before re-entering to Stop0 mode.
...
<!--
@startditaa doc/STMicroelectronics.example_hal_pwr_stop0-setup.png
  /--------------------\
  |                    |
  |       /------------+
  |       |    GPIO    |
...
![STMicroelectronics](doc/STMicroelectronics.example_hal_pwr_stop0-setup.png)
...
__Status LED__: This example does not follow the standard status LED pattern. When the LED is on, the system is in Run mode, otherwise it is in Stop0 mode.

__IOs__: In Stop0 mode, all I/O pins keep the same state as in the Run mode.

__Wake-up events__: All wake-up sources that can wake up the system from Stop0 mode are described in the reference manual.

__Memory retention__: When exiting from Stop0 mode, all register contents are preserved. All memory content is retained or lost depending on feature availability and application configurability. Refer to the reference manual of the MCU in use.

__BOR__: The BOR is always available in Stop0 mode.

__Wake-up flag__: Before entering Stop0 mode, we have to clear pending flags.

__Debugging low-power modes__: This example targets the lowest power consumption for Stop0 mode, and debugging can interfere. If you need to debug behavior around entry/exit of low-power modes, use the MCU debug features described in the reference manual (DBGMCU or SBS according products).
