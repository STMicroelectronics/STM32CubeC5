/**
  ******************************************************************************
  * file           : example.c
  * brief          : Enter and exit the Stop1 mode through an EXTI interrupt using the HAL API
  ******************************************************************************
  *
...
/** ########## Step 1 ##########
  ...
  * The application configures the wake-up source to wake-up the MCU from Stop1 mode.
  */
...
/** ########## Step 2 ##########
  * The application clears pending flags, preparing to enter Stop1 mode.
  * The application requests entry into Stop1 mode.
  * The application checks whether the system was in Stop1 mode.
  * The application reconfigures the system clock after exiting Stop1 mode.
  */
app_status_t app_process(void)
{
  ...
  /* Enter Stop1 low-power mode */
  HAL_PWR_EnterStopMode(HAL_PWR_LOW_PWR_MODE_WFI, HAL_PWR_STOP1_MODE);

  /* Execution resumes here when the MCU wakes up */
  if (HAL_PWR_GetPreviousSystemPowerMode() != HAL_PWR_SYSTEM_STOP1_MODE)
  {
    goto _app_process_exit;
  }

  /* Reconfigure the system clock configuration after exiting Stop1 mode */
  if (mx_rccx_init() != SYSTEM_OK)
  {
    goto _app_process_exit;
  }
