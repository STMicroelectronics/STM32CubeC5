...
# __Example: *hal_i3c_private_it_controller*__
...
The example implements the controller's code in I3C private mode, using interrupts (IT).
...
In this example, the controller and target exchange data using private, interrupt-driven (IT) I3C communication. The buffer `RxBuffer` is used for reception, while the controller alternates between two private transmit buffers (`BufferA` and `BufferB`) for sending data.
...
__Step 3__: The controller initiates communication in private IT mode by alternately sending a null-terminated string message (BufferA or BufferB) to the target and receiving data in RxBuffer. The attempt counter is reset at the start of each communication loop.
...
[INFO] Controller - Tx/Rx Buffers IDENTICAL. Transfer COMPLETED of I3C Two Boards Communication - Message A
[INFO] Controller - Tx/Rx Buffers IDENTICAL. Transfer COMPLETED of I3C Two Boards Communication - Message B
```
...
To test this example with the target, you can use the corresponding *hal_i3c_private_it_target* example pack.
...
<!--
@startuml
@startditaa{doc/example_hal_i3c_private_it_controller-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_hal_i3c_private_it_controller-setup](doc/example_hal_i3c_private_it_controller-setup.png)
...
- You can refer to the *hal_i3c_private_it_target* example pack to have a look at the target's board application.
