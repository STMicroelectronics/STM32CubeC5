...
# __Example: *hal_i3c_entdaa_it_target*__
...
How to participate in Dynamic Address Assignment (DAA/ENTDAA) as an I3C target using the HAL API in interrupt (IT) mode.
...
This example demonstrates how the target application participates in Dynamic Address Assignment (DAA/ENTDAA) using the STM32 HAL API in interrupt-driven (IT) mode.

__Initialization phase__: At main program start, the `mx_system_init()` function is called. It initializes the peripherals, nonvolatile memory (such as flash memory, NVM, or external memories), MPU regions (if applicable), the system clock, and the SysTick.

The application executes the following __example steps__:

__Step 1__: Configures and initializes the I3C instance and the NVIC. Registers the user callbacks for I3C target events.

__Step 2__: Performs the Dynamic Address Assignment (DAA) process: activates notification and waits for DAA completion.

__Step 3__: Retrieves CCC info and validates the assigned dynamic address and its validity after DAA completion.
...
[INFO] Step 3: Dynamic address received: 0xXX
[ERROR] Dynamic address assignment is not valid.
[ERROR] Dynamic address does not match expected value.
```
...
To test this example with the controller, you can use the corresponding *hal_i3c_entdaa_it_controller* example pack.

__Multi-target setup and preparation__:

To simulate multiple I3C targets for ENTDAA, prepare each board as follows:
For each target, set a unique `mipi_identifier` and the expected dynamic address in the firmware before connecting to the I3C bus:
  - `i3c_tgt_config_payload_entdaa.mipi_identifier` (unique per target)
  - `TARGET_CHECKED_DYNAMIC_ADDRESS` (expected dynamic address)
This ensures each target is uniquely identified and addressed during ENTDAA.
...
<!--
@startuml
@startditaa{doc/example_hal_i3c_entdaa_it_target-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_hal_i3c_entdaa_it_target-setup](doc/example_hal_i3c_entdaa_it_target-setup.png)
