...
How to handle transmit-receive transactions between two boards using the I3C bus protocol in direct mode with the HAL API.

This example implements the target's code in I3C direct mode.
...
In this example, the target application manages I3C data transfers in direct mode with the HAL API. The process is robust, with error handling and retry logic, and uses a stepwise approach:

__Initialization phase__: At main program start, the `mx_system_init()` function is called. It initializes the peripherals, nonvolatile memory (such as flash memory, NVM, or external memories), MPU regions (if applicable), the system clock, and the SysTick.

The application executes the following __example steps__:

__Step 1__: Configures and initializes the I3C instance and the NVIC. Registers the user callbacks for I3C target events: transfer error, and notification callback.

__Step 2__: Performs the Dynamic Address Assignment (DAA) process and verifies dynamic address assignment. Activates notification, waits for DAA completion, and checks the assigned address. The DAA process is executed only once per power-up.

__Step 3__: The target receives the SetMRL and SetMWL messages from the controller using DMA in direct mode. This initiates the communication loop for reliable data handling and error monitoring.
...
[INFO] Step 2: Dynamic address verification COMPLETED.
[INFO] - SETMWL processing and verification COMPLETED.
[INFO] - SETMRL processing and verification COMPLETED.
[ERROR] Target - Communication ERROR: hal_status = <value>, HAL_I3C_GetLastErrorCodes = <value>. TRYING AGAIN.
```
...
-  ENTDAA (Enter Dynamic Address Assignment) for the I3C target is configured with identifier 0xC6 and MIPI identifier 0x01

- The event and error interrupts of the I3C instance are configured and enabled in the NVIC.

To test this example with the controller, you can use the corresponding *hal_i3c_direct_dma_controller* example pack.
...
<!--
@startuml
@startditaa{doc/example_hal_i3c_direct_dma_target-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_hal_i3c_direct_dma_target-setup](doc/example_hal_i3c_direct_dma_target-setup.png)
...
Here are the points of attention for this specific example:
  ...
  2. For correct synchronization, always run the target application before running the controller. This ensures the target is ready to respond to the controller's direct I3C communication requests.

  3. A short delay is provided before starting the transmit/receive communication to allow the target to initialize and be ready for data exchange.
