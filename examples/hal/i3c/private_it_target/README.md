...
# __Example: *hal_i3c_private_it_target*__
...
How to handle an infinite number of transmit-receive transactions between two boards using the I3C bus protocol in private mode with the HAL API, using interrupt (IT) mode.

This example implements the target's code in I3C private mode, using interrupts.
...
In this example, the target application manages I3C data transfers in private mode using interrupt (IT) and the HAL API. The process is robust, with error handling and retry logic, and uses a stepwise approach:

__Initialization phase__: At main program start, the `mx_system_init()` function is called. It initializes the peripherals, nonvolatile memory (such as flash memory, NVM, or external memories), MPU regions (if applicable), the system clock, and the SysTick.

The application executes the following __example steps__:

__Step 1__: Configures and initializes the I3C instance and the NVIC. Registers the user callbacks for I3C target events: RX/TX transfer completed, transfer error, and notification callback.
...
__Step 3__: The target expects to receive a message as a null-terminated string from the controller board, in interrupt mode. A counter of attempts is reset when initiating the communication loop.
...
[INFO] Step 2: DAA process and dynamic address verification COMPLETED.
[INFO] Target - Message received and sent back: <data>
...
- DAA (Dynamic Address Assignment) for the I3C target is configured with identifier 0xC6 and MIPI identifier 0x01
...
<!--
@startuml
@startditaa{doc/example_hal_i3c_private_it_target-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_hal_i3c_private_it_target-setup](doc/example_hal_i3c_private_it_target-setup.png)
...
Here are the points of attention for this specific example:
  ...
  5. A short delay is provided before starting the transmit/receive communication to allow the target to initialize and be ready for data exchange.
