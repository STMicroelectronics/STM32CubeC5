...
# __Example: *hal_i3c_ibi_target*__
...
How to send In-Band Interrupts (IBI) as an I3C target using the HAL API in interrupt (IT) mode for wakeup demonstration.
...
This example demonstrates how the target application performs IBI signaling and interrupt-driven wakeup using the STM32 HAL API in interrupt (IT) mode.

__Initialization phase__: At main program start, the `mx_system_init()` function is called. It initializes the peripherals, nonvolatile memory (such as flash memory, NVM, or external memories), MPU regions (if applicable), the system clock, and the SysTick.

The application executes the following __example steps__:

__Step 1__: Initialize the I3C instance and NVIC. Register user callbacks for target notification and error events.

__Step 2__: Perform Dynamic Address Assignment (DAA): activate notification, wait for DAA completion (interrupt-driven), retrieve CCC info, and validate assigned dynamic address.

__Step 3__: Send IBI (In-Band Interrupt) request to the controller with a fixed payload. Wait for IBI process to complete (interrupt-driven).

The communication status is reported via the status LED and the variable `ExecStatus`.

__End of example__: If no error occurs, the target successfully receives and validates its dynamic address, and completes the IBI process. If an error occurs during DAA or IBI, the process stops and an error status is reported.
...
[INFO] Step 3: IBI process COMPLETED.
...
- IBI target configuration: payload size set to 3 byte and pending read MDB feature disabled.

To test this example with the controller, you can use the corresponding *hal_i3c_ibi_controller* example pack.
...
<!--
@startuml
@startditaa{doc/example_hal_i3c_ibi_target-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_hal_i3c_ibi_target-setup](doc/example_hal_i3c_ibi_target-setup.png)
