...
# __Example: *hal_i3c_ibi_controller*__
...
How to handle In-Band Interrupts (IBI) from multiple targets using the I3C bus protocol and STM32 HAL API in interrupt-driven (IT) mode.
...
This example demonstrates controller-side In-Band Interrupt (IBI) handling for multiple already dynamically addressed targets using the I3C protocol and STM32 HAL API in interrupt-driven (IT) mode. By default, the scenario is set up for two targets connected to the controller.
...
__Step 4__: Verifies the received IBI: CCC info, payload length, target address, and payload value to ensure correct processing.
...
[INFO] Step 4: All values received from the target are correct.
...
<!--
@startuml
@startditaa{doc/example_hal_i3c_ibi_controller-setup.png} -E -S
    /-------------------------\                     /-------------------------\
    |    /--------------------+                     +--------------\          |
...
![example_hal_i3c_ibi_controller-setup](doc/example_hal_i3c_ibi_controller-setup.png)
