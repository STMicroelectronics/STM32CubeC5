/**
  ******************************************************************************
  * file           : example.c
  * brief          : I3C Controller example for handling IBI requests using interrupt mode
  ******************************************************************************
  *
...
uint32_t TargetIndex; /* Index of target to store the different target capabilities */
...
app_status_t app_process(void)
{
  ...
  PRINTF("[INFO] Step 2: DAA process COMPLETED. Number of targets detected: %" PRIu32 "\n", (uint32_t)TargetCount);
  ...
  for (TargetIndex = 0; TargetIndex < TargetCount; TargetIndex++)
  {
    ...
    uint32_t bcr = HAL_I3C_GET_BCR(Targets_Descriptor[TargetIndex].target_bcr_dcr_pid);
    ...
    DeviceConf[TargetIndex].ibi_ack =
    ...
    DeviceConf[TargetIndex].ibi_payload =
    ...
    DeviceConf[TargetIndex].ctrl_role_req_ack =
    ...
    hal_status = HAL_I3C_CTRL_SetConfigBusDevices(pI3C, &DeviceConf[TargetIndex], 1U);
  ...
  hal_status = HAL_I3C_CTRL_ActivateNotification(pI3C, HAL_I3C_CTRL_NOTIFICATION_IBI);
  if (hal_status != HAL_OK)
  {
  ...
  PRINTF("[INFO] Step 4: All values received from the target are correct.\n");
...
/**
  ...
  * @Note: Dynamic address assignment (setting the address) is performed within this callback
  *        to ensure immediate handling and proper synchronization.
  */
static void CtrlTgtReqDynAddrCallback(hal_i3c_handle_t *hi3c, uint64_t target_payload)
{
