...
# __Example: *hal_i3c_sensor_private_it_controller*__

**Example version:** 2.0.0

[![User Manual](doc/read_the-UM.svg)](https://dev.st.com/stm32cube-docs/examples/arch-v1/en/index.html "An offline version is also available in the STM32Cube firmware package.")

How to handle as Controller data buffer transmission/reception between a board and the sensor LSM6DSV16X present on X-NUCLEO-IKS01A3, in private mode with the HAL API, using interrupt (IT) mode.
...
__Step 2__: The controller performs the one-time target address assignment and sensor configuration sequence by preparing the transfer context and sending the broadcast CCC commands.

__Step 3__: The controller completes the sensor setup by sending the direct `SETDASA` CCC, reconfiguring the I3C bus to 12.5 MHz, and transmitting the sensor configuration buffer.

__Step 4__: The controller repeatedly performs private write/read transfers to exchange data with the sensor, retrieves temperature, gyroscope, and accelerometer values, and displays them on the STLink console.
The sequence then returns to Step 4 indefinitely if no error occurs.
...
```text
[INFO] Step1: I3C initialized.
[INFO] Step2: CCC Broadcast command sent.
[INFO] Step3: Sensor is configured.
[INFO] -------------------LSM6DSO Measurement------------------
[INFO]  Temperature (Celsius degree): 24
[INFO]  GYR_X: -156, GYR_Y: 83, GYR_Z: 92
[INFO]  ACC_X: 537 , ACC_Y: 326, ACC_Z: 16488
```
...
![example_hal_i3c_sensor_private_it_controller-setup-setup](doc/example_hal_i3c_sensor_private_it_controller-setup.png)
...
Here are the points of attention for this specific example:
  ...
  2. Ensure that the sensor (target) is configured to handle the exact number of bytes expected from the controller during private command transactions.

  3. Verify that the sensor has received and acknowledged its dynamic address assignment before any private command transactions.
