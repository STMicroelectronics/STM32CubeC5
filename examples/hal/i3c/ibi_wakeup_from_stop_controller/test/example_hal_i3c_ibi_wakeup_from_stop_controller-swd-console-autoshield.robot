...
*** Settings ***
Documentation       Automatic test for example_hal_i3c_ibi_wakeup_from_stop_controller.
...
Example Step 3
    [Documentation]    Activates the notification IBI.
    Read Console Until    [INFO] Step 3: Activates the notification IBI COMPLETED.
    Send I3C Target IBI

Example Step 4
    [Documentation]    The controller enters stop mode and waits an interrupt to wakeup the MCU.
    Read Console Until    [INFO] Step 4

Example Step 5
    [Documentation]    The controller checks that the received data is correct.
    Read Console Until    [INFO] Step 5: All values received from the target are correct.
