...
# __Example: *hal_ccb_protected_ecc_scalar_mul_blob_creation*__
...
This example demonstrates how to securely create a protected ECC scalar multiplication key blob using the STM32 HAL CCB driver. A symmetric user key is used as a wrapping key to create the ECC scalar multiplication key blob according to ECC curve parameters. The resulting blob contains pointers to reserved output buffers for IV, Tag, and the wrapped scalar K, enabling secure ECC scalar multiplication operations only.
...
__Step 2__: Performs protected blob creation for ECC scalar multiplication, with the following sub-steps:

2.1 configures and performs symmetric key wrapping.
2.2 performs ECC private key wrapping for scalar multiplication.
2.3 verifies blob creation.
...
[INFO] Step 2.1: Symmetric key wrapping successfully COMPLETED.
[INFO] Step 2.2: ECC private key wrapping COMPLETED.
[INFO] Step 2.3: Blob creation for ECC scalar multiplication verification COMPLETED.
[INFO] Step 3: de-init.
```
...
The CCB peripheral is used to securely manage cryptographic operations, with a focus in this example on:
 ...
 - ccb_protected_ecc_scalar_mul_blob_use example
