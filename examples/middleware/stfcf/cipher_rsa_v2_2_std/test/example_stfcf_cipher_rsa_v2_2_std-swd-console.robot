...
*** Settings ***
Documentation       Automatic test for example_stfcf_cipher_rsa_v2_2_std.
...
Example Step 2
    [Documentation]    Encrypts a known message and decrypts the computed ciphertext using the regular private key representation.
    ...    2.1:    Compute directly the encrypted message using regular private key representation.
    ...    2.2:    Decryption of the ciphertext followed by the verification of the generated plaintext.
    Comment    Check application (app_process) runs without error
    Read Console Until    [INFO] Step 2.1: Message encryption COMPLETED.
    Read Console Until    [INFO] Step 2.2: Message decryption COMPLETED.
