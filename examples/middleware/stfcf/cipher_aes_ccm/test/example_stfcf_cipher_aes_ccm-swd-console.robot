...
*** Settings ***
Documentation       Automatic test for example_stfcf_cipher_aes_ccm.
...
Example Step 2
    [Documentation]    Encrypts and decrypts with associated data, then verifies.
    ...    2.1: Authenticated encryption with associated data (AEAD) COMPLETED.
    ...    2.2: Authenticated decryption with associated data (ADAD) COMPLETED.
    Comment    Check application (app_process) runs without error
    Read Console Until    [INFO] Step 2.1: Authenticated encryption with associated data (AEAD) COMPLETED.
    Read Console Until    [INFO] Step 2.2: Authenticated decryption with associated data (ADAD) COMPLETED.

Example Step 3
    [Documentation]    Deinitializes the RNG instance and Destroy the key to free up resources.
