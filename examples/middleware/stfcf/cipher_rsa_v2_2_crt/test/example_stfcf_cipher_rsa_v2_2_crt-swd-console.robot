...
*** Settings ***
Documentation       Automatic test for example_stfcf_cipher_rsa_v2_2_crt.
...
Example Step 2
    [Documentation]    Encrypts a known message and decrypts the computed ciphertext using the CRT private key representation.
    ...    2.1:    Encrypts a known message using CRT private key representation.
    ...    2.2:    Decrypts the computed ciphertext using CRT private key representation.
    Comment    Check application (app_process) runs without error
    Read Console Until    [INFO] Step 2.1: Encrypt a known message using CRT private key representation COMPLETED.
    Read Console Until
    ...    [INFO] Step 2.2: Decrypt the computed ciphertext using CRT private key representation COMPLETED.

Example Step 3
    [Documentation]    Deinitializes the RNG instance and destroys the PSA key before leaving the scenario.
