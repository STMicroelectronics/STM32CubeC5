...
*** Settings ***
Documentation       Automatic test for example_stfcf_sign_verify_rsa_v1_5_std.
...
Example Step 2
    [Documentation]    Computes the signature of the message using the regular private
    ...    key representation (modulus + exponent), so that generated signature can be compared to the known signature.
    ...    2.1: Compute directly the signature using regular private key representation,
    ...    2.2: Authenticated decryption of a known ciphertext,
    ...    followed by the verification of the generated plaintext.
    Comment    Check application (app_process) runs without error
    Read Console Until    [INFO] Step 2.1: Compute signature COMPLETED.
    Read Console Until    [INFO] Step 2.2: Verify message COMPLETED.
