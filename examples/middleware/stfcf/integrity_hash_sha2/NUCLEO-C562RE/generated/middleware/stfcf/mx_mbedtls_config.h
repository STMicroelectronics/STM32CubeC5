/**
  ******************************************************************************
  * @file    mx_mbedtls_config.h
  * @brief   Configuration options (set of defines)
  *          This set of compile-time options might be used to enable
...
#ifndef MX_MBEDTLS_CONFIG_H
#define MX_MBEDTLS_CONFIG_H
...
/**
  ...
  *      library/poly1305.c
  *
  * Some parts of the library might use multiplication of two unsigned 32-bit
  ...
  *
  * Note that depending on the compiler, this might decrease performance compared
...
/**
  ...
  * the date must be correct). This is used to verify the validity period of
  * X.509 certificates.
  *
...
/**
  ...
  *
  * This only works with GCC and Clang. With other compilers, you might use
...
/**
  ...
  * in mind that the function prototypes must remain the same.
  *
...
/**
  ...
  * mind that function prototypes must remain the same.
  *
  ...
  *            alternative implementations must use the RNG only for generating
  *            the ephemeral key and nothing else. If this is not possible, then
  *            MBEDTLS_ECDSA_DETERMINISTIC must be disabled and an alternative
  *            implementation must be provided for mbedtls_ecdsa_sign_det_ext().
  *
  */
...
/**
  ...
  * that function prototypes must remain the same.
  *
  ...
  * The mbedtls_internal_ecp_grp_capable function must return 1 if the
...
/**
  ...
  * must generally be checked in portable applications.
  *
...
/**
  ...
  * number of ECC operations a function might perform before pausing; see
...
/**
  ...
  *
  * \warning This is currently experimental, incomplete and therefore must not
...
/**
  ...
  * might result in a compromise of the long-term signing key. This is avoided by
...
/**
  ...
  *             If possible, it is recommended users must consider
...
/** MBEDTLS_PSA_CRYPTO_KEY_ID_ENCODES_OWNER
  ...
  *
  * Note that this option is meant for internal use only and might be removed
  * without notice.
  */
...
/** \def MBEDTLS_PSA_CRYPTO_BUILTIN_KEYS
  ...
  *
  * \warning This interface is experimental and might change or be removed
  * without notice.
  */
...
/** \def MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG
  ...
  *          enables.
  *
  * \note This option is experimental and might be removed without notice.
  */
...
/**
  ...
  * path). Alternatively, your platform might customize the header
...
/**
  ...
  *
  * This option might be enabled if all buffers passed to any PSA function reside
...
/** \def MBEDTLS_SSL_CLI_ALLOW_WEAK_CERTIFICATE_VERIFICATION_WITHOUT_HOSTNAME
  ...
  *
  * The third check might be safely skipped in some unusual scenarios,
  ...
  *
  * You must call mbedtls_ssl_set_hostname() with the expected server name
  ...
  *   of Mbed TLS, but might be insecure as explained above.
  *
...
/**
  ...
  *
  * If you need all error reporting you must enable this during debugging,
  * but remove this for production servers that must log as well.
  *
...
/**
  ...
  *       is enabled, a TLS handshake might call psa_crypto_init(), even
...
/**
  ...
  * amplifiers in DoS attacks against other hosts. It must always be enabled
...
/**
  ...
  * mbedtls_ssl_conf_export_keys_cb() must be used to implement
  * the key exporter described in Section 4.2 of RFC 5764 and RFC 5705
  * (this is implemented in the SSL example programs).
  * The resulting key must then be passed to an SRTP stack.
  *
...
/**
  ...
  * part of the library's API, so it might change or disappear at any time.
  *
...
/**
  ...
  *
  * Merely enabling this feature must not change the behavior of the product.
...
/**
  ...
  * will still continue to work as usual, so enabling this option must not
...
/**
  ...
  * mechanism). You might still freely enable additional `MBEDTLS_XXX` symbols
  * in mbedtls_config.h.
  *
  ...
  *          files.
  *          Future minor releases of Mbed TLS might make minor changes to those
  *          symbols, but we will endeavor to provide a transition path.
  *          Nonetheless, this option is considered mature enough to use in
  *          production, as long as you accept that you might need to make
...
/**
  ...
  *       implementation, that alternate implementation might opt to not support
...
/**
  ...
  * is used.
  *
  * \warning When using PSA, the user must call `psa_crypto_init()` before
...
/**
  ...
  * option are experimental, and must not be used in production.
  *
...
/**
  ...
  *             interface.
  *
  * \warning    This feature is not thread-safe, and must not be used in a
...
/**
  ...
  * That name is deprecated, but might still be used as an alternative form for this
  * option.
  *
...
/**
  ...
  * This name is now deprecated, but might still be used as an alternative form for
  * this option.
  */
...
/**
  ...
  * Caller:
  *
  * Requires: (MBEDTLS_CIPHER_C or MBEDTLS_USE_PSA_CRYPTO) and
  *           (MBEDTLS_GCM_C or MBEDTLS_CCM_C or MBEDTLS_CHACHAPOLY_C)
  */
...
/**
  ...
  * enable this option if more than one thread might use any part of
...
/**
  ...
  *
  * If defined, this is a header which will be included instead of
  * `"psa/crypto_platform.h"`. This file must declare the same identifiers
  ...
  *       must review the changes in the original file whenever they
  *       upgrade Mbed TLS.
  *
  * This macro is expanded after an <tt>\#include</tt> directive. This is a popular but
  * non-standard feature of the C language, so this feature is only available
...
/**
  ...
  *
  * If defined, this is a header which will be included instead of
  * `"psa/crypto_struct.h"`. This file must declare the same identifiers
...
/** \} name SECTION: General configuration options */
  ...
  * relevant header files and must suffice for the regular use cases.
  *
...
/** \def MBEDTLS_PLATFORM_STD_CALLOC
  ...
  * If the product is 0, the function might either return NULL or a valid pointer to an array of size 0 which is a valid input to the deallocation function.
  * An uninitialized #MBEDTLS_PLATFORM_STD_CALLOC always fails, returning a null pointer.
...
/** \def MBEDTLS_CHECK_RETURN
  ...
  * to indicate that its return value must be checked. It must
...
/** \def MBEDTLS_IGNORE_RETURN
  *
  * This macro requires one argument, which must be a C function call.
...
/**
  ...
  *
  *       A future version might reevaluate the prioritization of DRBG mechanisms.
  */
...
/** \def MBEDTLS_PSA_KEY_SLOT_COUNT
  ...
  * set up. This might change in future versions of the library.
  *
  * Currently, the library traverses of the whole table on each access to a
  * persistent key. Therefore large values might cause poor performance.
  *
  * This option has no effect when #MBEDTLS_PSA_CRYPTO_C is disabled.
  */
...
/**
  * \def MBEDTLS_SRAM_ERASED_ON_RESET
  *
  * Improve boot efficiency for OEMiRoT.
  */
/* #define MBEDTLS_SRAM_ERASED_ON_RESET */
...
/** \def MBEDTLS_SSL_CID_TLS1_3_PADDING_GRANULARITY
  ...
  *       a power of two must be preferred.
  */
...
/** \def MBEDTLS_SSL_DTLS_MAX_BUFFERING
  ...
  *
  * This must be at least 9/8 * MBEDTLS_SSL_IN_CONTENT_LEN
  * to account for a reassembled handshake message of maximum size,
  * together with its reassembly bitmap.
  *
  * A value of 2 * MBEDTLS_SSL_IN_CONTENT_LEN (32768 by default)
  * must be sufficient for all practical situations as it allows
...
/**
  ...
  * at the same pace. The typical accuracy of an RTC crystal is plus or minus 100 to plus or minus 20 parts
