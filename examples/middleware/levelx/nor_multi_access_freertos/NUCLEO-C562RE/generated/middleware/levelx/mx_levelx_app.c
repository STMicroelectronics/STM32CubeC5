...
{
  UCHAR data_written[LX_NOR_SECTOR_SIZE];
  UCHAR data_read[LX_NOR_SECTOR_SIZE];
} task_data_t;
...
UINT app_levelx_init(VOID)
{
  ...
  PRINTF("[INFO] Step 2: Initialize LevelX NOR flash\r\n");
  /* Initialize LevelX NOR flash */
...
VOID main_task_entry(VOID *args)
{
  ...
  /** ########## Step 4 ##########
    * Format and open LevelX NOR flash.
    */

  PRINTF("[INFO] Step 4: Format and open LevelX NOR flash\r\n");
  ...
  /* Check the NOR flash driver format status.  */
  ...
  /* Check the NOR flash driver open status.  */
  if (status != LX_SUCCESS)
  {
  ...
  /** ########## Step 6 ##########
    * Close the NOR flash driver.
    */

  PRINTF("[INFO] Step 6: Close the NOR flash driver\r\n");

  /* Waiting for either xBit2 et xBit3 to be set. */
  if (xEventGroupWaitBits(xEventGroup, xBit2 | xBit3, pdTRUE, pdTRUE, TIMEOUT) == (xBit2 | xBit3))
  {
    /* Close the NOR flash driver.  */
    status = lx_nor_flash_close(&NORFlashObj);

    /* Check the NOR flash driver close status.  */
    if (status != LX_SUCCESS)
    {
...
VOID app_task_entry_one(VOID *args)
{
  ...
  for (j = 0; j < 5; j++)
  {
    ...
    status = lx_nor_flash_sector_write(&NORFlashObj, app_nor_sector, (VOID *)pdata->data_written);
    if (status != LX_SUCCESS)
    {
    ...
    status = lx_nor_flash_sector_read(&NORFlashObj, app_nor_sector, (VOID *)pdata->data_read);
    if (status != LX_SUCCESS)
    {
      xTaskNotify(BackgroundTask, NOTIFICATION_VALUE_ERROR, eSetValueWithOverwrite);
    }
    ...
    if (memcmp(pdata->data_written, pdata->data_read, LX_NOR_SECTOR_SIZE) != 0)
    {
      /* Error writing the data, notify the background task.  */
      xTaskNotify(BackgroundTask, NOTIFICATION_VALUE_ERROR, eSetValueWithOverwrite);
    }
    ...
    lx_nor_flash_sector_release(&NORFlashObj, app_nor_sector);
