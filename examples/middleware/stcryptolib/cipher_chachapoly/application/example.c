/**
  ******************************************************************************
  * file           : example.c
  * brief          : This example describes how to use the STM32 Cryptographic library to perform an
  *                  authenticated encryption and a verified decryption using
...
cmox_chachapoly_handle_t p_chacha_ctx;
...
/** Extract from IETF
  ...
  * 064  6f 6e 74 68 73 20 61 6e 64 20 6d 61 79 20 62 65  onths and can be
...
/** ########## Step 2 ##########
  * Demonstrates encryption and decryption of a known ciphertext using
...
app_status_t app_process(void)
{
  ...
  /** ########## Step 2.1 ##########
    * Encrypts and verifies directly a plainText and a tag using the single call method.
    */

  /* Computes directly the ciphertext and the tag passing all the needed parameters */
  ...
  /* Verify that generated data are the expected ones */
  if (memcmp(Expected_Ciphertext, Computed_Ciphertext, sizeof(Expected_Ciphertext)) != 0)
  {
    goto _app_process_exit;
  }

  /* Verify that generated authentication tag is the expected one */
  ...
  /** ########## Step 2.2 ##########
    * Decrypts and verifies directly a ciphertext and a tag using the single call method.
    */
  ...
  PRINTF("[INFO] Step 2.2: Decryption and verification of a known ciphertext COMPLETED.\n");
  ...
  /** ########## Step 3.1 ##########
    * Sets the encryption context for Chacha20-Poly1305.
    */

  /* Construct a cipher context that is configured to perform Chacha20-Poly1305 encryption operations */
  cipher_ctx = cmox_chachapoly_construct(&p_chacha_ctx, CMOX_CHACHAPOLY_ENC);
  if (cipher_ctx == NULL)
  {
    goto _app_process_exit;
  }
  ...
  PRINTF("[INFO] Step 3.1: Set the encryption context for Chacha20-Poly1305 algorithm COMPLETED.\n");

  /** ########## Step 3.2 ##########
    * Encrypts and verifies directly a plainText and a tag using the multiple API calls method.
    */
  ...
  PRINTF("[INFO] Step 3.2: Encryption and verification using multiple API calls method COMPLETED.\n");

  /** ########## Step 3.3 ##########
    * Sets the decryption context for Chacha20-Poly1305.
    */

  /* Construct a cipher context that is configured to perform Chacha20-Poly1305 decryption operations */
  cipher_ctx = cmox_chachapoly_construct(&p_chacha_ctx, CMOX_CHACHAPOLY_DEC);
  if (cipher_ctx == NULL)
  {
    goto _app_process_exit;
  }
  ...
  PRINTF("[INFO] Step 3.3: Set the decryption context for Chacha20-Poly1305 algorithm COMPLETED.\n");

  /** ########## Step 3.4 ##########
    * Decrypts and verifies directly a ciphertext and a tag using the multiple API calls method.
    */
  ...
  PRINTF("[INFO] Step 3.4: Decryption and verification using multiple API calls method COMPLETED.\n");
...
app_status_t app_deinit(void)
{
  ...
  PRINTF("[INFO] Step 4: STM32 Cryptographic library deinitialization COMPLETED.\n");
