...
*** Settings ***
Documentation       Automatic test for example_stcryptolib_auth_verify_aes_cmac.
...
Example Step 2
    [Documentation]    Demonstrates how to use the STM32 Cryptographic library to authenticate and to verify
    ...    a message using the AES CMAC algorithm.
    ...    2.1: Computes and verifies the message using single API call method.
    ...    2.2: Computes and verifies the message using multiple API calls method.
    Comment    Check application (app_process) runs without error
    Read Console Until    [INFO] Step 2.1: Compute and verify the message using single API call method COMPLETED.
    Read Console Until    [INFO] Step 2.2: Compute and verify the message using multiple API calls method COMPLETED.
