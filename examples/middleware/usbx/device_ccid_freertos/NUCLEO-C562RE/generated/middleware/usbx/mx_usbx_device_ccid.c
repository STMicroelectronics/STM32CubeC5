...
ULONG usbd_ccid_data_rates[USBD_CCID_N_DATA_RATES] =
{
...
/**
  ...
  *         deactivate the card slot and get the ATR from ICC
...
UINT usbd_ccid_icc_power_on(ULONG slot, UX_DEVICE_CLASS_CCID_MESSAGES *io_msg)
{
  ...
  io_msg -> ux_device_class_ccid_messages_rdr_to_pc_length = sizeof(usbx_ccid_atr_t0) + UX_DEVICE_CLASS_CCID_MESSAGE_HEADER_LENGTH;
...
UINT usbd_ccid_xfr_block(ULONG slot, UX_DEVICE_CLASS_CCID_MESSAGES *io_msg)
{
  ...
  if (usbx_ccid_protocol == 0)
  {
    ...
    usbd_ccid_apdu_process(&apdu_cmd, &apdu_rsp);
...
/**
  * @brief  Function implementing usbd_ccid_thread_entry.
  * @param  none
  * @retval none
  */
...
/* APDU related API */
/**
  * @brief  usbd_ccid_apdu_enable_chv
  *         Enable CHV
  * @param  apdu_cmd: pointer to apdu command
  * @param  apdu_rsp: pointer to apdu response
  * @retval none
  */
static VOID usbd_ccid_apdu_enable_chv(CCID_APDU_Command *apdu_cmd, CCID_APDU_Response *apdu_rsp)
{
  UINT i;
...
/**
  * @brief  usbd_ccid_apdu_select
  *         Select a file about the logical card model
  * @param  apdu_cmd: pointer to apdu command
  * @param  apdu_rsp: pointer to apdu response
  * @retval none
  */
static VOID usbd_ccid_apdu_select(CCID_APDU_Command *apdu_cmd, CCID_APDU_Response *apdu_rsp)
{
...
/**
  * @brief  usbd_ccid_apdu_verify_change
  *         Change to the CHV or VERIFY CHV
  * @param  apdu_cmd: pointer to apdu command
  * @param  apdu_rsp: pointer to apdu response
  * @retval none
  */
static VOID  usbd_ccid_apdu_verify_change(CCID_APDU_Command *apdu_cmd, CCID_APDU_Response *apdu_rsp)
{
  UINT i;
  UCHAR *chv;
...
/**
  * @brief  usbd_ccid_apdu_read_data
...
/**
  * @brief  usbd_ccid_apdu_process
  *         Manage APDU command and response
  * @param  apdu_cmd: pointer to apdu command
  * @param  apdu_rsp: pointer to apdu response
  * @retval none
  */
VOID  usbd_ccid_apdu_process(CCID_APDU_Command *apdu_cmd, CCID_APDU_Response *apdu_rsp)
{
  switch (apdu_cmd->INS)
  {
    case 0x28: /* ENABLE CHV1  A0 28 00 01 08 CHV1 value */
      usbd_ccid_apdu_enable_chv(apdu_cmd, apdu_rsp);
      return;

    case 0xA4: /* SELECT  A0 A4 00 00 02 File ID */
      usbd_ccid_apdu_select(apdu_cmd, apdu_rsp);
      return;

    case 0x20: /* VERIFY  A0 20 00 xx 08 CHV Value */
    case 0x24: /* CHANGE  A0 24 00 xx 10 Old CHV, New CHV */
      usbd_ccid_apdu_verify_change(apdu_cmd, apdu_rsp);
      return;

    case 0xC0: /* GET RESPONSE A0 C0 00 00    xx */
    case 0xB0: /* READ BINARY  A0 B0 xx xx    xx */
      usbd_ccid_apdu_read_data(apdu_cmd, apdu_rsp);
      return;
