...
VOID usbd_cdc_acm_activate(VOID *cdc_acm_instance)
{
  ...
  if (HAL_UART_Receive_IT(p_uart_handle, (uint8_t *)UserTxBufferFS, 1) != HAL_OK)
  {
...
void HAL_UART_RxCpltCallback(hal_uart_handle_t *huart, uint32_t size_byte, hal_uart_rx_event_types_t rx_event)
{
  ...
  if (HAL_UART_Receive_IT(p_uart_handle, (uint8_t *)UserTxBufferFS + UserTxBufPtrIn, 1) != HAL_OK)
  {
    /* Transfer error in reception process */
    while (1);
  }
