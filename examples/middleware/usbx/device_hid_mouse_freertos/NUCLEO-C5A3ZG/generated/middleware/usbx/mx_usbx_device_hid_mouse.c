...
/**
  * @brief  Function implementing usbd_hid_mouse_thread_entry.
  *         Run HID mouse task
  * @param  none
  * @retval none
  */
VOID usbd_hid_mouse_thread_entry(VOID *args)
{
  ...
  while (1)
  {
    ...
    if ((device->ux_slave_device_state == UX_DEVICE_CONFIGURED) && (p_hid_mouse != UX_NULL))
    {
      /* sleep for 10ms */
