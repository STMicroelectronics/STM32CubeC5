...
hal_spi_handle_t *mx_spi1_init(void)
{
  ...
  if (HAL_SPI_SetTxDMA(&hSPI1, &hLPDMA1_CH0) != HAL_OK)
  {
    return NULL;
  }
  ...
  if (HAL_SPI_SetRxDMA(&hSPI1, &hLPDMA1_CH1) != HAL_OK)
  {
    return NULL;
  }

  /* Enable the interrupt for SPI */
  HAL_CORTEX_NVIC_SetPriority(SPI1_IRQn, HAL_CORTEX_NVIC_PREEMP_PRIORITY_7, HAL_CORTEX_NVIC_SUB_PRIORITY_0);
