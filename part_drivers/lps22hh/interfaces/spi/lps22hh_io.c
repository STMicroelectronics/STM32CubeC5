/**
  ******************************************************************************
  * @file    lps22hh_io.c
  * @brief   LPS22HH IO driver file for SPI.
  ******************************************************************************
  * @attention
  *
...
/**
  * @brief  Initialize the LPS22HH IO SPI flavor.
...
/**
  * @brief  The write register function for LPS22HH IO SPI flavor.
  * @param  pio the IO instance object
...
int32_t lps22hh_io_write_reg(lps22hh_io_t *pio, const uint8_t reg, const uint8_t *pdata, uint16_t len)
{
  ...
  if (HAL_SPI_Transmit(pio->hspi, &reg_in, 1, LPS22HH_SPI_POLL_TIMEOUT) != HAL_OK)
  {
    ret = -1;
  }
  ...
  if (!ret)
  {
    if (HAL_SPI_Transmit(pio->hspi, pdata, len, LPS22HH_SPI_POLL_TIMEOUT) != HAL_OK)
    {
      ret = -1;
    }
  }
...
/**
  * @brief  The read register function for LPS22HH IO SPI flavor.
  * @param  pio the IO instance object
...
int32_t lps22hh_io_read_reg(lps22hh_io_t *pio, const uint8_t reg, uint8_t *pdata, uint16_t len)
{
  ...
  if (!ret)
  {
    if (HAL_SPI_Receive(pio->hspi, pdata, len, LPS22HH_SPI_POLL_TIMEOUT) != HAL_OK)
    {
      ret = -1;
    }
  }
